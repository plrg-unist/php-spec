...
Unsupported, tool errors, timeouts and interrupted campaigns never pass.

## Accepted code and frozen integration

Request/GLOBALS/top-level magic: code652d10b7, author d3dfb87a, compiler ca3ac883,
independent eed5bf8d. Reviewed845/1d206c8e binds263 request literals as260 private
parent plus3 consumer successor,44 ordinary magic, author6dense2,619 and prior
...
Infrastructure9fc1ce9f/a934749a, independent56bec412: current **846/14d17662**.
Only source-compiler failure retention/deadline changed and the13-reference
preflight script was added;844 prior inputs match. Private roots must run
`scripts/private-semantics-references.py ROOT --copy-missing` before freezing,
...
`.tools/runtime5-quiet-integration/pending-preflight.json` retains five old core
controls: quiet-property, quiet-nullsafe, quiet-static-property, nested-return,
context/iterable-nullsafe-ref. Four parser controls remain separately historical.
Seven old environment cases, four isset GLOBALS controls and guarded GLOBALS now
have explicit-request witnesses; original absent-request Unsupported states are
unchanged. Two later-eval ENV/REQUEST callback witnesses remain source-pending;
pure callback replacement is independently checked. Their original full states
are bound by archive SHA9e54f1c9 and the lossless2261-file runtime history.

Runtime's private first calls root: `.tools/runtime5-calls-first`.
Mutable interface: `.tools/runtime5-calls-design/runtime-interface.md`.
Author native/lint originals28: `originals-rlidrvaj/originals.json` under that
design directory; trace originals6: `trace-originals-k6w0mu34`. Nine further trace/config controls are in `trace-context-7prtrak6/originals.json`: pin
exception trace settings0:15, mutated by-value argument9, unsetNULL, rebound2,
omitted defaultf(), surplusf(9,2,'a'). Conditional redeclaration is runtime Fatal
error with an active stack but no Uncaught/thrown suffix; too-few arguments enter
the callee trace and use declaration line. These are native originals, not passes.
Independent originals **f5cbac10**:
`coverage/semantics/call-ownership-independent-originals.json`, raw
`.tools/review7-call-ownership-4pbsg63_`. Six cases cover caller-owned temporary,
suspended by-ref iterator (17233), argument surviving global unset (2), returned
singleton-ref COW (17) versus shared global-ref COW (777), and callee fatal output
BA with actual native trace. All share explicit authored request facts.

The proposed surgical design keeps active ENV/CVS/SYMBOLS and moves the main
table into optional GLOBALTABLE during functions. Saved main frame owns no
duplicate table; saved function frames own their locals. Request-wide HTTPROOTS/
ACTIVATED remain global. Inactive callers' HELD, pending arguments, result values
and iterator roots must remain visible to heap traversal. Capture the returned
value/reference as an owner before callee cleanup; restore callers exactly once.
Callback replacement must reach the global table and preserve old aliases.

Routing follows actual compiler fetch designation, including known computed
superglobal names. Dynamic same-spelling names remain local. `global $v` binds a
local alias; later local reference/foreach rebinding can detach it from the global
bucket. New CODE.GLOBALS descriptors need missing/duplicate/wrong-occurrence
integrity controls. Trace arguments use current slots, not frozen entry values;
actual argc differs from declared/defaulted parameters. Surplus arguments own
values. Capture error trace/source context before teardown and bind effective
exception trace configuration to pinned facts. Runtime is capturing unset/rebind
and argument truncation originals before final descriptor choice.

Compiler5 private candidate `.tools/compiler5-calls` has20 native/lint compiler
phase/message comparisons; originals are `originals/records.json`, current fixture
`check-z29wjnpd`. It is mutable, not independently accepted; additional body/CV/
autoglobal/magic/name/export controls are being prepared. Changed modules30/33/45/
46/71/77/new78-function-compiler/modules remain private.

Compiler5 supplies real function declarations/body code/signatures/CVS/lexical
contexts and request-wide activation facts. Every body resets proper header,
loop, magic and strictness context. Callsite scalar-argument strictness and body
return strictness differ. Preserve unique source occurrence identity across
units/bodies. New source rules require original phase/line evidence, meaningful
ownership/resumption tests and independent review; no helper-only calls milestone.
Follow CALLS-ACTIVATION-PLAN.md and maintained COMPILER-HANDOFF.md.

The top-level nested-foreach return witness prints13, returns7 and skips trailing
output; cleanup must preserve surviving aliases. It belongs in the first coherent
