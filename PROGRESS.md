...
The canonical implementation is **1026/0ec57507**, with accepted arrow capture
...
First-class named function callables and pipe semantics are reviewed in private
candidates but are **not installed on 1026**. The first-class successor has 17
maintained outcomes (13 normal, 3 PHP errors, 1 static rejection), with
conversion TMP/write/send distinctions. Pipe has 12 maintained outcomes (7
normal, 5 PHP errors) and a separately reviewed 30,980-entry classified syntax
gate for the pipe arrow grouping prerequisite. Their fingerprints and raw audits
remain separate from this baseline; selected builtin, method, array-callable and
constant-context forms are still explicit dependencies.

## Next work

Install the reviewed first-class increment, pipe grouping/static prerequisite,
and pipe runtime/compiler increment as small commits after this checkpoint.
