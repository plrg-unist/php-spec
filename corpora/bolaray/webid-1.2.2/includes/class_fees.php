...
class fees
{
            ...
            while (!feof($fp)) {
                ...
                if (strcmp($resl, 'VERIFIED') == 0) {
                    ...
                    if ($payment_status == 'Completed') {
                        ...
                        self::callback_process($custom_id, $aucid, $useraccid, $winnerid, $fee_type, $payment_amount);
                    }
    ...
    private function callback_process($custom_id, $aucid, $useraccid, $winnerid, $fee_type, $payment_amount, $currency = null)
    {
        switch ($fee_type) {
            case 1: // add to account balance
                $addquery = '';
                if ($this->system->SETTINGS['fee_disable_acc'] == 'y') {
                    $query = "SELECT suspended, balance FROM " . $this->DBPrefix . "users WHERE id = :user_id";
                    $params = array(
            ...
            case 2: // pay for an item
                $query = "UPDATE " . $this->DBPrefix . "winners SET paid = 1 WHERE id = :id";
                $params = array(
                    array(':id', $winnerid, 'int')
                );
                $this->database->query($query, $params);
                break;
            ...
            case 4: // pay auction fee (live mode)
                ...
                $params = array(
                    array(':useracc_id', $useraccid, 'int')
                );
                $this->database->query($query, $params);
