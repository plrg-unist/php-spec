...
function ws_users_getList($params, &$service)
{
    ...
    if (!empty($params['user_id'])) {
        $where_clauses[] = 'u.id IN('. implode(',', $params['user_id']) .')';
    }

    if (!empty($params['username'])) {
        $where_clauses[] = 'u.username LIKE \''.pwg_db_real_escape_string($params['username']).'\'';
    }
    ...
    if (!empty($params['filter'])) {
        ...
        $filter_where_clause = '('.'u.username LIKE \'%'.
    ...
    if (!empty($params['exclude'])) {
        $where_clauses[] = 'u.id NOT IN('. implode(',', $params['exclude']) .')';
    }
