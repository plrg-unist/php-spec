...
SELECT count(*)
FROM '.USERS_TABLE.'
WHERE upper(email) = upper(\''.$mail_address.'\')
'.(is_numeric($user_id) ? 'AND id != \''.$user_id.'\'' : '').'
;';
...
SELECT id AS id,
       password AS password
  FROM '.USERS_TABLE.'
  WHERE username = \''.pwg_db_real_escape_string($username).'\'
;';
  ...
  SELECT id AS id,
         password AS password
    FROM '.USERS_TABLE.'
    WHERE email = \''.pwg_db_real_escape_string($username).'\'
    ;';
