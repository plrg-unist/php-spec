...
?>
  <a class="std" href="<?php echo $_SERVER['PHP_SELF']."?month_no=".$monthchange."&amp;year=".$yearchange ?>">
  <img src="templates/<?php echo $style ?>/images/leftarrow.gif" border="0" alt="<?php echo date('M', mktime(0, 0, 0, $monthchange, 1, $yearchange))." ".$yearchange ?>"></img>
  </a></td>
  <td class="tdheading" colspan="5"><?php echo $lang["$monthfulltext"]." ".$year ?></td>
  <td class="tdheading">
  <?php $monthchange = $month_no+1;
...
?>
  <a class="std" href="<?php echo $_SERVER['PHP_SELF']."?month_no=".$monthchange."&amp;year=".$yearchange ?>">
  <img src="templates/<?php echo $style ?>/images/rightarrow.gif" border="0" alt="<?php echo date('M', mktime(0, 0, 0, $monthchange, 1, $yearchange))." ".$yearchange ?>"></img></a>


  </td></tr>
<tr>
  <td class="tdsunday"><?php echo $lang["shortsunday"] ?>
  </td><td class="tdday"><?php echo $lang["shortmonday"] ?>
  </td><td class="tdday"><?php echo $lang["shorttuesday"] ?>
  </td><td class="tdday"><?php echo $lang["shortwednesday"] ?>
  </td><td class="tdday"><?php echo $lang["shortthursday"] ?>
  </td><td class="tdday"><?php echo $lang["shortfriday"] ?>
  </td><td class="tdday"><?php echo $lang["shortsaturday"] ?>
  </td>
</tr>
<tr>
<?php
...
for ($date_of_mth = 1; $date_of_mth <= $day_in_mth; $date_of_mth++) {
    ...
    if ($date_no ==  gmtdate('j', time(), $timezone) &&  $month_no == gmtdate('n', time(), $timezone)) {
        if ($row > 0) {
            $linkdate ="<a class=\"std\" href=\"".$_SERVER['PHP_SELF']."?mode=viewdate&amp;date_no=".$date_no."&amp;month_no=".$month_no."&amp;year=".$year."\">".$date_no."</a>";
        } else {
            $linkdate  = $date_no ;
        }

        ?>
     <td class="tdtoday"><?php echo $linkdate ?></td>
     <?php } else {
         if ($row > 0) {
             $linkdate ="<a class=\"std\" href=\"".$_SERVER['PHP_SELF']."?mode=viewdate&amp;date_no=".$date_no."&amp;month_no=".$month_no."&amp;year=".$year."\">".$date_no."</a>";
         } else {
         ...
         ?>
   <td class="tddate"><?php echo $linkdate ?></td>
   <?php  }


    if ($day_of_wk == 6) {  ?></tr><tr><?php }
    if ($day_of_wk < 6 && $date_of_mth == $day_in_mth) {
        for ($i = $day_of_wk ; $i < 6; $i++) {
            ?><td class="tddate">&nbsp;</td><?php }
        ?></tr><?php
    }
}
?>
