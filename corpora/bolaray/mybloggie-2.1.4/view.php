...
while ($row = $db->sql_fetchrow($result)) {
    ...
    if (!empty($row['email'])) {
        $com_email =" <a class=\"std\" href=\"mailto:".$row['email']."\">".$lang['Email']."</a> : ";
    } else {
        $com_email = " | ";
    }
    if (!empty($row['home'])) {
        $com_home = $lang['Track_from']." <a class=\"std\"  href=\"".$commenthome1."\">".$row['poster']."</a>";
    } else {
        $com_home = "";
    }
...
while ($row = $db->sql_fetchrow($result)) {
    ...
    if (!empty($row['home'])) {
        $com_home ="<a class=\"std\"  href=\"".$commenthome1."\">".$lang['Home']."</a>";
    } else {
        $com_home = "";
    }
    $comments  =  $row['comments'] ;
