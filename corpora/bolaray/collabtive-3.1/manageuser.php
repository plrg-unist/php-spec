...
if ($action == "loginerror") {
    ...
    foreach ($alluser as $all) {
        if (!chkproject($all['ID'], $pid)) {
            array_push($users, $all);
        }
    }

    $members = $projectObj->getProjectMembers($pid, 14);
    $pro = $projectObj->getProject($pid);
    ...
    $vCard = new vCard($theuser["locale"]);
...
elseif ($action == "projectMembers") {
    ...
    $members = $projectObj->listProjectMembers($pid, $limit, $offset);

    $projectMembers["count"] = $projectObj->countMembers($pid);
