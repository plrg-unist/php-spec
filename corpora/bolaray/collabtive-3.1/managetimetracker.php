...
if ($action == "add") {
    ...
    $excelFile = fopen(CL_ROOT . "/files/" . CL_CONFIG . "/ics/user-$uid-timetrack.csv", "w");

    $line = array($strproj, $strtask, $strcomment, $strday, $strstarted, $strended, $strhours);
    fputcsv($excelFile, $line);
    if (!empty($cleanGet["start"]) and !empty($cleanGet["end"])) {
        $tracks = $tracker->getUserTrack($uid, $cleanGet["project"], $cleanGet["task"], $cleanGet["start"], $cleanGet["end"], false);
    } else {
        $tracks = $tracker->getUserTrack($uid, $cleanGet["project"], $cleanGet["task"], 0, 0, false);
    }
    if (!empty($tracks)) {
        foreach ($tracks as $track) {
            $hrs = round($track["hours"], 2);
    ...
    fclose($excelFile);
    $loc = $url . "files/" . CL_CONFIG . "/ics/user-$uid-timetrack.csv";
    header("Location: $loc");
    ...
    $uname = $conn->query("SELECT name FROM user WHERE ID = {$conn->quote($uid)}")->fetch();
    $uname = $uname[0];
