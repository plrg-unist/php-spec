...
class project extends databaseModel
{
     ...
     */
    public function add($name, $desc, $end, $budget, $assignme = 0)
    {
        global $conn, $mylog;
        ...
        $insStmt = $conn->prepare("INSERT INTO projekte (`name`, `desc`, `start`, `end`, `status`, `budget`) VALUES (?,?,?,?,1,?)");
        $ins = $insStmt->execute(array($name, $desc, time(), $end, (float)$budget));
    ...
    public function addProjectFromExcel($inputFileName)
    {
        ...
        for ($i = 1; $i < count($sheetData); $i++) {
            ...
            if ($task) {
                ...
                {
                    $taskid = $taskObj->add($task[2], $task[3], $task[0], $task[4], $tasklist["ID"], $projectId);
                }
     ...
     * @return bool
     */
    public function edit($id, $name, $desc, $end, $budget)
    {
        global $conn, $mylog;
        $end = strtotime($end);
        $id = (int)$id;
     ...
     * @return bool
     */
    public function deassign($user, $id)
    {
        global $conn, $mylog;
     ...
     */
    public function getProjects($status = 1, $limit = 10, $offset = 0)
    {
        global $conn;
     ...
     */
    public function getMyProjects($user, $status = 1, $offset = 0, $limit = 10, $orderBy = "projekt", $orderDirection = "DESC")
    {
        global $conn;
     ...
     */
    public function countProjects($status = 1)
    {
        global $conn;
     ...
     */
    public function countMyProjects($user, $status = 1)
    {
        global $conn;
     ...
     */
    public function getMyProjectIds($user)
    {
        global $conn;
     ...
     */
    public function getProjectMembers($project, $lim = 10, $paginate = true)
    {
     ...
     * @return array $members Projektmitglieder
     */
    public function listProjectMembers($project, $limit = 10, $offset = 0)
    {
        global $conn;
        $project = (int)$project;
        $limit = (int)$limit;
     ...
     */
    public function countMembers($project)
    {
        global $conn;
        $project = (int) $project;
     ...
     */
    public function getProjectFolders($project)
    {
        global $conn;
        $project = (int)$project;
