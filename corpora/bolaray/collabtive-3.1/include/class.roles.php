...
class roles
{
     ...
     * @return bool
     */
    public function add($name, array $projects, array $tasks, array $milestones, array $messages, array $files, array $timetracker, array $chat, array $admin)
    {
        global $conn;
     ...
     * @return bool
     */
    public function edit($id, $name, array $projects, array $tasks, array $milestones, array $messages, array $files, array $timetracker, array $chat, array $admin)
    {
        global $conn;
        $id = (int) $id;
        $projects = serialize($projects);
    ...
    public function assign($role, $user)
    {
        ...
        if ($chk > 0) {
            ...
            $ins = $insStmt->execute(array($role, $usr));
        } else {
            $insStmt = $conn->prepare("INSERT INTO roles_assigned (user,role) VALUES (?,?)");
            $ins = $insStmt->execute(array($usr, $role));
        }
     ...
     * @return bool
     */
    public function deassign($role, $user)
    {
        global $conn;
        $role = (int) $role;
        $user = (int) $user;
     ...
     */
    public function getAllRoles($limit = 10, $offset = 0)
    {
        $limit = (int) $limit;
        $offset = (int) $offset;
     ...
     */
    public function sanitizeArray($inarr)
    {
        if (!is_array($inarr)) {
            $inarr = array();
        }
