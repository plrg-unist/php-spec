...
class message
{
     ...
     * @return bool
     */
    public function add($project, $title, $text, $user, $username, $replyto, $milestone)
    {
        global $conn, $mylog;
     ...
     */
    public function getMessage($id, $recurseReplies = true)
    {
        global $conn;
        $id = (int)$id;
     ...
     * @return array $message Eigenschaften der Nachricht
     */
    public function getLatestMessages($limit = 25)
    {
        global $conn;
        $limit = (int)$limit;
     ...
     */
    public function getProjectMessages($project, $limit = 0, $offset = 0)
    {
        global $conn;
        $project = (int)$project;
        $limit = (int)$limit;
        ...
        $messages = array();
        if ($limit > 0) {
            $projectMessagesStmt = $conn->prepare("SELECT messages.ID FROM messages
     ...
     */
    public function getUserMessages($user, $limit = 10, $offset = 0)
    {
        global $conn;
        $user = (int)$user;
        $limit = (int)$limit;
     ...
     */
    public function getPublicUserMessages($user)
    {
        global $conn;
        $user = (int)$user;
        $myTasks = array();
     ...
     */
    public function countProjectMessages($project)
    {
        global $conn;
        $project = (int)$project;

        $messages = array();
        $sel1 = $conn->prepare("SELECT COUNT(*) FROM messages
                  LEFT JOIN messages_assigned ON messages.ID = messages_assigned.message
                  WHERE messages_assigned.message IS NULL
                  AND project = ?
                  AND replyto = 0");
     ...
     */
    public function countUserMessages($user)
    {
        global $conn;
        $user = (int)$user;
    ...
    public function assignToUser($user, $message)
    {
        global $conn;
        $message = (int)$message;
        $user = (int)$user;
     ...
     * @return bool
     */
    public function attachFile($fileId, $messageId, $project = 0)
    {
        global $conn;
        $fileId = (int)$fileId;
