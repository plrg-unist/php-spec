...
$Qdownload->bindInt(':orders_products_download_id', $id);
...
if (($Qdownload->valueInt('download_maxdays') != 0) && ($download_timestamp <= time())) {
    die;
}
// Die if remaining count is <=0
if ($Qdownload->valueInt('download_count') <= 0) {
    die;
}
...
$Qupdate->bindInt(':orders_products_download_id', $id);
$Qupdate->execute();
