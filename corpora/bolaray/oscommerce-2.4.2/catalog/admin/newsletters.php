        ...
        case 'confirm_send':
            ...
            if ($Qcheck->fetch() !== false) {
                if ($Qcheck->valueInt('locked') < 1) {
                    switch ($action) {
                        case 'delete': $error = OSCOM::getDef('error_remove_unlocked_newsletter');
                            break;
                        case 'new': $error = OSCOM::getDef('error_edit_unlocked_newsletter');
                            break;
                        case 'send': $error = OSCOM::getDef('error_send_unlocked_newsletter');
                            break;
                        case 'confirm_send': $error = OSCOM::getDef('error_send_unlocked_newsletter');
                            break;
                    }
...
<?php
  if ($action == 'new') {
      ...
      ?>
      <tr><?php echo HTML::form('newsletter', OSCOM::link(FILENAME_NEWSLETTERS, 'page=' . $_GET['page'] . '&action=' . $form_action));
...
<?php
      ...
      while ($Qnewsletters->fetch()) {
          ...
          ?>
                ...
                <td class="dataTableContent" align="center"><?php if ($Qnewsletters->valueInt('status') === 1) {
                ...
                <td class="dataTableContent" align="center"><?php if ($Qnewsletters->valueInt('locked') > 0) {
                    echo HTML::image(OSCOM::linkImage('icons/locked.gif'), OSCOM::getDef('icon_locked'));
                } else {
                    echo HTML::image(OSCOM::linkImage('icons/unlocked.gif'), OSCOM::getDef('icon_unlocked'));
                } ?></td>
                <td class="dataTableContent" align="right"><?php if (isset($nInfo) && is_object($nInfo) && ($Qnewsletters->valueInt('newsletters_id') === (int)$nInfo->newsletters_id)) {
                    echo HTML::image(OSCOM::linkImage('icon_arrow_right.gif'), '');
                } else {
                    echo '<a href="' . OSCOM::link(FILENAME_NEWSLETTERS, 'page=' . $_GET['page'] . '&nID=' . $Qnewsletters->valueInt('newsletters_id')) . '">' . HTML::image(OSCOM::linkImage('icon_info.gif'), OSCOM::getDef('image_icon_info')) . '</a>';
                } ?>&nbsp;</td>
              </tr>
<?php
      }
