    ...
    switch ($action) {
        case 'setflag':
            if (($_GET['flag'] == '0') || ($_GET['flag'] == '1')) {
                if (isset($_GET['rID'])) {
                    tep_set_review_status($id, $_GET['flag']);
                }
            }
...
<?php
  if (($action == 'edit') || ($action == 'preview')) {
      ...
      <tr>
        <td class="main"><strong><?php echo OSCOM::getDef('entry_rating'); ?></strong>&nbsp;<?php echo OSCOM::getDef('text_bad'); ?>&nbsp;<?php for ($i=1; $i<=5; $i++) {
            echo HTML::radioField('reviews_rating', $i, $rInfo->reviews_rating == $i) . '&nbsp;';
        } echo OSCOM::getDef('text_good'); ?></td>
      </tr>
      <tr>
...
<?php
          ...
          ?></td>
                <td class="dataTableContent" align="right"><?php if ((is_object($rInfo)) && ($Qreviews->valueInt('reviews_id') === (int)$rInfo->reviews_id)) {
                    echo HTML::image(OSCOM::linkImage('icon_arrow_right.gif'));
                } else {
                    echo '<a href="' . OSCOM::link(FILENAME_REVIEWS, 'page=' . $_GET['page'] . '&rID=' . $Qreviews->valueInt('reviews_id')) . '">' . HTML::image(OSCOM::linkImage('icon_info.gif'), OSCOM::getDef('image_icon_info')) . '</a>';
                } ?>&nbsp;</td>
              </tr>
<?php
      }
