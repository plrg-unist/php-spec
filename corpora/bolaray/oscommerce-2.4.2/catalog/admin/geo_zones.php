    ...
    <table border="0" width="100%" cellspacing="0" cellpadding="2">
      <tr>
        <td width="100%"><table border="0" width="100%" cellspacing="0" cellpadding="0">
          <tr>
            <td class="pageHeading"><?php echo OSCOM::getDef('heading_title');
if (isset($_GET['zone'])) {
...
<?php
      ...
      while ($Qzones->fetch()) {
          ...
          ?>
                ...
                <td class="dataTableContent" align="right"><?php if (isset($sInfo) && is_object($sInfo) && ($Qzones->valueInt('association_id') === (int)$sInfo->association_id)) {
                    echo HTML::image(OSCOM::linkImage('icon_arrow_right.gif'), '');
                } else {
                    echo '<a href="' . OSCOM::link(FILENAME_GEO_ZONES, 'zpage=' . $_GET['zpage'] . '&zID=' . $_GET['zID'] . '&action=list&spage=' . $_GET['spage'] . '&sID=' . $Qzones->valueInt('association_id')) . '">' . HTML::image(OSCOM::linkImage('icon_info.gif'), OSCOM::getDef('image_icon_info')) . '</a>';
                } ?>&nbsp;</td>
              </tr>
<?php
      }
      ?>
              ...
              <tr>
                <td class="smallText" align="right" colspan="3"><?php if (empty($saction)) {
                    echo HTML::button(OSCOM::getDef('image_back'), 'fa fa-chevron-left', OSCOM::link(FILENAME_GEO_ZONES, 'zpage=' . $_GET['zpage'] . '&zID=' . $_GET['zID'])) . HTML::button(OSCOM::getDef('image_insert'), 'fa fa-plus', OSCOM::link(FILENAME_GEO_ZONES, 'zpage=' . $_GET['zpage'] . '&zID=' . $_GET['zID'] . '&action=list&spage=' . $_GET['spage'] . '&' . (isset($sInfo) ? 'sID=' . $sInfo->association_id . '&' : '') . 'saction=new'));
                } ?></td>
              </tr>
            </table>
<?php
...
<?php
      ...
      while ($Qzones->fetch()) {
          ...
          ?>
                ...
                <td class="dataTableContent" align="right"><?php if (isset($zInfo) && is_object($zInfo) && ($Qzones->valueInt('geo_zone_id') === (int)$zInfo->geo_zone_id)) {
                    echo HTML::image(OSCOM::linkImage('icon_arrow_right.gif'));
                } else {
      ...
      ?>
              ...
              <tr>
                <td class="smallText" align="right" colspan="2"><?php if (!$action) {
                    echo HTML::button(OSCOM::getDef('image_insert'), 'fa fa-plus', OSCOM::link(FILENAME_GEO_ZONES, 'zpage=' . $_GET['zpage'] . (isset($zInfo) ? '&zID=' . $zInfo->geo_zone_id : '') . '&action=new_zone'));
                } ?></td>
              </tr>
            </table>
<?php
        ...
        default:
            if (isset($sInfo) && is_object($sInfo)) {
                ...
                if (tep_not_null($sInfo->last_modified)) {
                    $contents[] = array('text' => OSCOM::getDef('text_info_last_modified') . ' ' . DateTime::toShort($sInfo->last_modified));
                }
            }
            break;
    }
