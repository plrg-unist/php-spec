...
if (tep_not_null($action)) {
    switch ($action) {
        case 'setflag':
            tep_set_specials_status($id, $_GET['flag']);
        ...
        case 'deleteconfirm':
            ...
            $OSCOM_Db->delete('specials', ['specials_id' => (int)$specials]);
...
<?php
  if (($action == 'new') || ($action == 'edit')) {
      ...
      ?>
      <tr><form name="new_special" action="<?php echo OSCOM::link(FILENAME_SPECIALS, tep_get_all_get_params(array('action', 'info', 'sID')) . 'action=' . $form_action); ?>" method="post"><?php if ($form_action == 'update') {
      ...
      echo HTML::hiddenField('products_price', (isset($sInfo->products_price) ? $sInfo->products_price : '')); ?></td>
          </tr>
          <tr>
...
<?php
          ...
          ?></td>
                <td class="dataTableContent" align="right"><?php if (isset($sInfo) && is_object($sInfo) && ($Qspecials->valueInt('specials_id') === (int)$sInfo->specials_id)) {
                    echo HTML::image(OSCOM::linkImage('icon_arrow_right.gif'), '');
                } else {
                    echo '<a href="' . OSCOM::link(FILENAME_SPECIALS, 'page=' . $_GET['page'] . '&sID=' . $Qspecials->valueInt('specials_id')) . '">' . HTML::image(OSCOM::linkImage('icon_info.gif'), OSCOM::getDef('image_icon_info')) . '</a>';
                } ?>&nbsp;</td>
      </tr>
<?php
      }
