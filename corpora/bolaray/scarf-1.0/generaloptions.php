...
while ($row = mysql_fetch_assoc($result)) {
    ...
    } elseif (! strcmp("boolean", $row['type'])) {
...
while ($row = mysql_fetch_assoc($result)) {
    ...
    foreach ($row as $name => $value) {
        ...
        } elseif (!strcmp($name, "user_id")) {
            // Do nothing
        } else {
            print "<td><input name='{$id}_$name' value='$value' size='18'></td>\n";
        }
    }
    print "</tr>";
}
