...
<?php if (isset($session_id) && ! is_admin()) {
    ...
    while ($row = mysql_fetch_array($result)) {
        ...
        if (isset($session_id) && $session_id == $row[session_id]) {
            print " selected='selected' ";
        }
...
<tr><td>&nbsp</td><td><input type='submit' name='submit' value='<?php if (isset($_GET['paper_id'])) {
    print "Save Changes";
} else {
    print "Add Paper";
} ?>'></tr></tr>
...
<?php
    ...
    if (empty($_POST['title'])) {
        die("A title is required. $error");
    }
    if (empty($_POST['abstract'])) {
        die("An abstract is required. $error");
    }
    if (0 == count($_POST['authors'])) {
        die("At least one author is required. $error");
    }
    $session = $_POST['session'];
    if ((string)$session !== (string)(int)$session) {
        die("Something has gone wrong with your session selection. Either you don't have any sessions created, or the submission failed. $error");
    }
    $session = (int)$session;
    if (0 == mysql_num_rows(query("SELECT * FROM sessions WHERE session_id='$session'"))) {
        die("There isn't a session with that id. $error");
    }
