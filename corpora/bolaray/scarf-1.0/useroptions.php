...
if (isset($_POST['submit'])) {
    ...
    query("UPDATE users SET firstname='" . mysql_real_escape_string($_POST['firstname']) . "', lastname='" . mysql_real_escape_string($_POST['lastname']) . "', showemail='$showemail', affiliation='" . mysql_real_escape_string($_POST['affiliation']) . "' WHERE user_id='$user_id'");
    logToFile("$email changed their personal information");
    if (!empty($_POST['password'])) {
        if (strcmp($_POST['password'], $_POST['password2'])) {
            die("You're passwords don't match");
        } else {
    ...
    if (strcmp($email, $_POST['email'])) {
        if (strcmp($_POST['email'], $_POST['email2'])) {
            die("Your email addresses don't match.");
        }
...
You may log in at: " . getBaseLink() . "/login.php", "From: scarf-do-not-reply@" . $_SERVER['HTTP_HOST']);
        ...
        query("UPDATE users SET password='" . md5($pass) . "', email='$email' WHERE user_id='$user_id'");
    }
    print "<h3>Changes saved</h3>";
}
$result = query("SELECT firstname, lastname, showemail, affiliation, email FROM users WHERE user_id='" . $user_id . "'");
...
<tr><td>Show Email Publically:</td><td><input name='showemail' type='checkbox' <?php if($showemail) {
    print "checked='true'";
} ?>></td></tr>
