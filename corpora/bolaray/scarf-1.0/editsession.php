...
if (isset($_GET['action'])) {
    ...
    } elseif (!strcmp($_GET['action'], "confirmdelete")) {
...
if (isset($_POST['submit'])) {
    if ((-1 == (int) $_POST['starthour']) || (-1 == (int) $_POST['startminute']) || (0 == (int) $_POST['month']) || (0 == (int) $_POST['day']) || empty($_POST['year'])) {
        ...
        if ($endtimestamp < $timestamp) {
            die("Your endtime was after your starttime.");
        }
...
<option value="0"></option>
<?php for ($i=1; $i < 32; $i++) {
    print "<option value='$i' " . ($starttime != 0 && $i == (int) date("j", $starttime) ? "selected='selected'" : "") . ">$i</option>";
}  ?>
</select>
<input name="year" value='<?php if ($starttime != 0) {
    print date("Y", $starttime);
} ?>'/>
</tr>
...
<?php for ($i=0; $i < 24; $i++) {
    print "<option value='$i' " . ($starttime != 0 && $i == (int) date("G", $starttime) ? "selected='selected'" : "") . ">$i</option>";
}  ?>
</select>
:<select name="startminute">
<option value="-1"></option>
<?php for ($i=0; $i < 60; $i+=5) {
    print "<option value='$i' " . ($starttime != 0 && $i == (int) date("i", $starttime) ? "selected='selected'" : "") . ">$i</option>";
} ?>
</select>
End:<select name="endhour">
<option value="-1"></option>
<?php for ($i=0; $i < 24; $i++) {
    print "<option value='$i' " . ($starttime != 0 && $i == (int) date("G", $endtime) ? "selected='selected'" : "") . ">$i</option>";
}  ?>
</select>
:<select name="endminute">
<option value="-1"></option>
<?php for ($i=0; $i < 60; $i+=5) {
    print "<option value='$i' " . ($starttime != 0 && $i == (int) date("i", $endtime) ? "selected='selected'" : "") . ">$i</option>";
} ?>
</select>
</td></tr>
</table>
