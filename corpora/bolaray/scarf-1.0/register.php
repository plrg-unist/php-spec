...
<table>
<tr><td>Email Address <?php if (is_admin()) {
    print "(Leave blank if you don't know their email)";
} ?>:</td><td><input name='email'></td></tr>
...
<?php
    ...
    if (! strcmp($_SERVER['PHP_SELF'], $_SERVER['HTTP_REFERER']) && ! is_admin()) {
    ...
    if (!empty($_POST['email'])) {
        ...
        if (mysql_num_rows($result) > 0) {
            die("Sorry, that email is already taken");
        }
    } elseif (is_admin()) {
...
You may login to your new account at the above website address.  We recommend that you change your password to something that is a little easier to remember.",
            "From: scarf-do-not-reply@" . $_SERVER['HTTP_HOST']
        );
