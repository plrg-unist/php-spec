				...
				&& $_REQUEST['PASSWORD'] !== '' )
			{
				...
				if ( $user_type === 'staff' )
				{
					...
					DBQuery( "UPDATE staff SET PASSWORD='" .
						...
						WHERE STAFF_ID='" . (int) $user_id_a . "'
						AND SYEAR='" . Config( 'SYEAR' ) . "'" );

					// If admin, send notification email to server admin.
					if ( $user_profile == 1 )
					{
						_notifyServerAdminPasswordReset( $user_id_a );
					}
				}
 ...
 */
function _sendPasswordResetEmail($user_id_a, $user_id, $user_type, $email )
{
	global $DatabaseType;
	...
	if ( $user_type === 'staff' )
	{
        $last_login_a = DBGetOne( "SELECT
            CAST(CURRENT_TIMESTAMP + INTERVAL " . ( $DatabaseType === 'mysql' ? '2 hour' : "'2 hour'" ) . " AS char(19)) AS LAST_LOGIN" );
		// Update Last login = now + 2 hours.
		DBQuery( "UPDATE staff
			SET LAST_LOGIN='" . $last_login_a . "'
			WHERE STAFF_ID='" . (int) $user_id_a . "'" ); // CURRENT_TIMESTAMP + interval '2 hour'.
	}
	elseif ( $user_type === 'student' )
	{
		// Update Last login = now + 2 hours.
