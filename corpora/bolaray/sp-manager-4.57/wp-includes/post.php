...
function wp_delete_post($postid = 0, $force_delete = false)
{
    ...
    if (is_post_type_hierarchical($post->post_type)) {
        ...
        if ($children) {
            $parent_where1 = $parent_where;
            $parent_where1 += array( 'post_type' => $post->post_type );
            $wpdb->update($wpdb->posts, $parent_data, $parent_where1);
        }
    }
    ...
    $parent_where2 = $parent_where;
    $parent_where2 += array( 'post_type' => 'attachment' );
    $wpdb->update($wpdb->posts, $parent_data, $parent_where2);
...
function wp_unique_post_slug($slug, $post_id, $post_status, $post_type, $post_parent)
{
    ...
    if ('attachment' === $post_type) {
         ...
         */
        $post_name_check = $wpdb->get_var($wpdb->prepare("SELECT post_name FROM $wpdb->posts WHERE post_name = %s AND post_type IN ( %s, 'attachment' ) AND ID != %d AND post_parent = %d LIMIT 1", $slug, $post_type, $post_id, $post_parent));
        ...
        if ($post_name_check
            ...
            do {
                $alt_post_name   = _truncate_post_slug($slug, 200 - (strlen($suffix) + 1)) . "-$suffix";
                $post_name_check = $wpdb->get_var($wpdb->prepare("SELECT post_name FROM $wpdb->posts WHERE post_name = %s AND post_type IN ( %s, 'attachment' ) AND ID != %d AND post_parent = %d LIMIT 1", $alt_post_name, $post_type, $post_id, $post_parent));
                $suffix++;
            } while ($post_name_check);
            $slug = $alt_post_name;
        }
        ...
        if ($post_name_check
            ...
            do {
                $alt_post_name   = _truncate_post_slug($slug, 200 - (strlen($suffix) + 1)) . "-$suffix";
                $post_name_check = $wpdb->get_var($wpdb->prepare("SELECT post_name FROM $wpdb->posts WHERE post_name = %s AND post_type = %s AND ID != %d LIMIT 1", $alt_post_name, $post_type, $post_id));
                $suffix++;
            } while ($post_name_check);
            $slug = $alt_post_name;
        }
