...
add_filter('wpfh_sub_projects', array($spdm_sub_projects_new ,'project_dropdown'), 10, 3);
...
{
    public function getBreadCrumbRecursor($pid)
    {
    ...
    public function getBreadCrumbRecursor_email($pid)
    {
    ...
    public function getBreadCrumb_email($pid)
    {
        global $wpdb;
    ...
    public function emailBreadCrumb($message, $r, $r_project, $r_cats)
    {
    ...
    public function makeInput($extra_js)
    {
        global $user_ID;
		...
		<div id="sp_cdm_breadcrumbs"><span><a href="javascript:sp_cdm_load_project(0);"   class="sp_cdm_home"">'.__("Home", "sp-client-document-manager").'</a> &raquo;</span> <span id="sp_cdm_breadcrumbs_subs">  </span>  </div>';
...
public function recursor($datas, $parent = 0, $depth = 0, $chosen=0)
{
    if($depth > 5000) {
        return '';
...
public function sub_project_form($form)
{
    ...
    $html = '<div class="remodal" data-remodal-id="folder" data-remodal-options="{ \'hashTracking\': false }">
		...
		<p>
		<label>
		'.sp_cdm_folder_name() .' '.__("Name", "sp-client-document-manager").': </label>
		<input  id="sub_category_name" type="text" name="project-name"  style="width:200px !important">
		</p>
		<p>
		<label>
		'.sp_cdm_folder_name() .' '.__("Parent", "sp-client-document-manager").':</label>
		 <select name="pid" id="sub_category_parent"><option value="0">None</option>'. $this->recursor($options).'</select>
		 </p>
		 
		<p><input type="submit" value="'.__("Add", "sp-client-document-manager").' '.sp_cdm_folder_name() .'" name="add-project" onclick="sp_cu_add_project()"></p>
...
public function recursor2($array, $r = 0, $p = null, $chosen = 0)
{
...
public function parents($uid)
{
    ...
    $r_projects_query_search = apply_filters('sp_cdm_projects_query', '', $uid);
...
public function parents_all($id)
{
...
public function project_dropdown($html, $chosen= false, $uid= 0)
{
    global $wpdb,$current_user;
    unset($html);
    $html = '';
    ...
    $html .= '<select name="pid" class="pid_select'.$requird.'"><option value="">'.__("None", "sp-client-document-manager").'</option>
...
public function project_dropdown_all($html, $id, $chosen= false)
{
    global $wpdb,$current_user;
    unset($html);
    $html = '';
    ...
    $html .= '<select name="pid" class="pid_select'.$requird.'"><option value="0">'.__("None", "sp-client-document-manager").'</option>
	  '. $this->recursor($options, 0, 0, $chosen).'</select>';
...
public function project_dropdown_replace()
{
    global $wpdb,$current_user;
    unset($html);
    ...
    $html .= '<option value="">'.__("None", "sp-client-document-manager").'</option>'. spdm_sub_projects_new::recursor($options).'';
