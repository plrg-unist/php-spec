...
{
    public function filelist_edit_save($update)
    {
        if(cdm_var('file-url')) {
    ...
    public function filelist_edit_content($html, $r)
    {
        if($r[0]['url'] != '') {
            $html .='<p><strong>'.__('Link URL', 'sp-client-document-manager').'</strong><br> <input style="width:100%"  placeholder="' . __("Link URL", "sp-client-document-manager") . '"  type="text" name="file-url"  value="'.stripslashes($r[0]['url']).'"></p>';
        }
        return $html;
    }
    public function filelist_link($link, $r)
    {
    ...
    public function viewfile_image_list($img, $r)
    {
    ...
    public function viewfile_image($info_left_column, $r)
    {
    ...
    public function viewfile_delete_button_rev($html, $r)
    {
