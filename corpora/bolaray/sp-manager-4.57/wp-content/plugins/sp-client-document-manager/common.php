...
function cdm_set_upload_dir($upload)
{
...
function cdm_esc_html($content)
{

    $allowed_html = wp_kses_allowed_html('post');
    $allowed_html['script'] = 1;
    $allowed_html['a'] = 1;

    return wp_check_invalid_utf8($content);
}
...
function cdm_recycle_folder($id, $restore = false)
{
    global $wpdb;
...
function cdm_recycle_file($id, $restore = false)
{
    global $wpdb;
...
function cdm_delete_folder($project_id = null, $force = false)
{
    ...
    if ((($current_user->ID == $r[0]['uid'] or cdmFindLockedGroup($current_user->ID, $r[0]['uid']) == true) && get_option('sp_cu_user_delete_disable') != 1) or current_user_can('manage_options') or cdm_folder_permissions($project_id) == 1 or $force == true) {
        ...
        #delete the project
        sp_cdm_write_log('Deleted folder: ' . $r[0]['name'] . '');
        $wpdb->query($wpdb->prepare("DELETE FROM " . $wpdb->prefix . "sp_cu_project WHERE id = %d", $project_id));
    } else {
        sp_cdm_write_log('Error: Failed removing folder: ' . $r[0]['name'] . '');
    }
}
function cdm_delete_file($file_id = false, $force = false)
{
    ...
    sp_cdm_write_log('Deleted file: ' . $r[0]['name'] . '');
    cdm_event_log($r[0]['pid'], $current_user->ID, 'folder', '<strong style="color:red">' . __('Deleted File: ' . $r[0]['name'] . '</strong>', 'sp-client-document-manager') . '');
    $wpdb->query($wpdb->prepare("DELETE FROM " . $wpdb->prefix . "sp_cu WHERE id = %d ", $file_id));
...
function sp_cdm_show_file_linked($html)
{
    ...
    if ($fid != '') {
        ...
        if (cdm_folder_permissions($r[0]['pid']) == 1) {
            ...
            cdm_event_log($r[0]['id'], $current_user->ID, 'file', '' . __('<strong>Security Exception:</strong> Tried viewing this file through a share link without permissions.', 'sp-client-document-manager') . '');
        }
    }
    return $html;
}
...
{
    return '' . get_site_url() . '/?sp-cdm-link=' . base64_encode(intval($fid)) . '&view=1';
}
function sp_cdm_folder_link($fid)
{
    ...
    return '' . get_site_url() . '/?cdm-f=' . base64_encode(intval($fid)) . '&view=1'. $dashboard.'';
}
...
function cdm_file_size($file)
{
    ...
    if ($filesize <= 0) {
        return $filesize = ''.__("Unknown file size", "sp-client-document-manager").'';
    } else {
    ...
    if ($cache_permission == false) {
        ...
        if (class_exists('sp_cdm_groups_addon')) {
            ...
            if (class_exists('sp_cdm_groups_addon_projects')) {
                ...
                #	print_r($user_roles);
                if (count($user_roles) > 0 && is_array($folder_perm_roles)) {
...
function cdm_delete_permission($pid)
{
    ...
    if ($cache_permission === false) {
        ...
        if (class_exists('sp_cdm_groups_addon')) {
            ...
            $user_roles        = $current_user->roles;
            if(!is_array($folder_perm_roles)) {
                $folder_perm_roles = array();
            }
            if (count($user_roles) > 0) {
                foreach ($user_roles as $key => $role) {
                    if (in_array($role, $folder_perm_roles)) {
...
if (!function_exists('sp_cdm_category_value')) {
    ...
    function sp_cdm_category_name()
    {
        if (get_option('sp_cu_cat_text') != '') {
            ...
            $cat = __("Category", "sp-client-document-manager");
        }
        return $cat;
    }
}
    ...
    function sp_cdm_folder_name($type = 0)
    {
        if ($type == 1) {
            if (get_option('sp_cu_folder_name_plural') == '') {
                return __("Folders", "sp-client-document-manager");
            } else {
                return stripslashes(esc_attr(get_option('sp_cu_folder_name_plural')));
            }
        } else {
            if (get_option('sp_cu_folder_name_single') == '') {
                return __("Folder", "sp-client-document-manager");
            } else {
                return stripslashes(esc_attr(get_option('sp_cu_folder_name_single')));
            }
        }
    }
}
...
function sp_cdm_get_project_name($id)
{
    global $wpdb;
    if ($id == '' or $id == 0) {
        return __('Root', 'sp-client-document-manager');
    }
...
function sp_client_upload_filename($user_id)
{
    ...
    $extra = esc_attr(get_option('sp_cu_filename_format'));
...
function sp_client_upload_filename_new($filename, $user_id)
{
    ...
    if (get_option('sp_cu_filename_format') == '') {
        ...
        $r          = $wpdb->get_results("SELECT *
									 ...
									 FROM " . $wpdb->prefix . "users  where id = $user_id", ARRAY_A);
        $filename   = esc_attr(get_option('sp_cu_filename_format'));
