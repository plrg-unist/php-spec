...
function sp_client_upload_options_vendors()
{
    ...
    if(cdm_var('submit-vendor')) {
        ...
        if (wp_verify_nonce(cdm_var('cdm_nonce_save_vendor'), 'cdm_nonce_save_vendor') == false) {
            exit('Security Error');
        }
    ...
    if(cdm_var('delete-vendor')) {
        ...
        $wpdb->query("DELETE FROM ".$wpdb->prefix."options WHERE option_id = ".intval(cdm_var('delete-vendor'))."");
...
'.__("Name:", "sp-client-document-manager").' <input type="text" name="name">  '.__("Email:", "sp-client-document-manager").' <input type="text" name="email"> <input type="submit" name="submit-vendor" value="Add new vendor">
...
<th>'.__("Name", "sp-client-document-manager").'</th>

<th>'.__("Email", "sp-client-document-manager").'</th>

<th>'.__("Delete", "sp-client-document-manager").'</th>
...
<td><a href="admin.php?page=sp-client-document-manager-vendors&delete-vendor='.$vendors[$i]['option_id'].'">'.__("Delete", "sp-client-document-manager").'</a></td>
