...
class cdm_recycle_bin
{
    ...
    public function restore_recycle_bin()
    {
        global $wpdb;
        $json = array();
    ...
    public function empty_recycle_bin()
    {
        ...
        if(count($xfolders)>0) {
            ...
            $json['message']  = __('Rerunning', 'sp-client-document-manager');
            echo json_encode($json);
            die();
        ...
        $json['rerun'] = 0;
        $json['message']  = __('Emptied recycle bin!', 'sp-client-document-manager');
        sp_cdm_write_log($json['message']);
        echo json_encode($json);
        die();
    }
    public function restore_object()
    {
        ...
        if($type == 'folder') {
            ...
            $json['message'] = __('Folder Restored  from recycle bin!', 'sp-client-document-manager');
        } elseif($type == 'file') {
            $id = cdm_var('id');
            $where['id'] = $id;
            $wpdb->update("" . $wpdb->prefix . "sp_cu", $update, $where);
            $json['message'] = __('Files Restored from recycle bin!', 'sp-client-document-manager');
        }
        sp_cdm_write_log($json['message']);
        echo json_encode($json);
        die();
    ...
    public function delete_object()
    {
        ...
        if($type == 'folder') {
            ...
            $json['message'] = __('Folder Deleted  from recycle bin!', 'sp-client-document-manager');
        } elseif($type == 'file') {
            $id = cdm_var('id');
            cdm_delete_file($id);
            $json['message'] = __('File Deleted  from recycle bin!', 'sp-client-document-manager');
        }
        sp_cdm_write_log($json['message']);
        echo json_encode($json);
        die();
    }
    public function view_recycled_item()
    {
        ...
        if($type == 'file') {
            ...
            echo '<div class="cdm-recycle-bin-menu"><a href="#" class="button cdm-recycle-function" data-type="file" data-id="'.$id.'" data-method="delete">'.__('Delete File Permanently', 'sp-client-document-manager').'</a>    <a href="#" class="button cdm-recycle-function" data-type="file" data-id="'.$id.'" data-method="restore">'.__('Restore File', 'sp-client-document-manager').'</a></div>
				
		
			';
            echo '
		<div class="sp_su_project">	
	<strong>' . __("Recycled Date", "sp-client-document-manager") . ': </strong> ' . date("F j, Y", strtotime($r[0]['recycle_date'])) . '	
	</div>
	<div class="sp_su_project">	
	<strong>' . __("Automatic Removal Date", "sp-client-document-manager") . ': </strong>  ' . date("F j, Y", strtotime($r[0]['recycle_date'].'  + '.get_option('sp_cu_recycle_bin_retention', 30).' days')) . '	
	</div>
		<div class="sp_su_project">	
	<strong>' . __("File Name", "sp-client-document-manager") . ': </strong> ' . stripslashes($r[0]['name']) . '	
	</div>
	<div class="sp_su_project">
	' . date('F jS Y h:i A', strtotime($r[0]['date'])) . ' &bull; File ID: #' . $r[0]['id'] . ' 
	</div>
	<div class="sp_su_project">	
	<strong>' . __("File", "sp-client-document-manager") . ': </strong> ' . stripslashes($r[0]['file']) . '	
	</div>';

            $owner = get_userdata($r[0]['uid']);
            echo '<div class="sp_su_project">
		
		<strong>' . __("File Owner", "sp-client-document-manager") . ': </strong> '.apply_filters('sp_cdm/file/owner_name', $owner->display_name, $r[0]) .'
	...
	';
            ...
            echo '<div class="cdm-recycle-bin-menu"><a href="#" class="button cdm-recycle-function" data-type="folder" data-id="'.$id.'" data-method="delete">'.__('Delete Folder Permanently', 'sp-client-document-manager').'</a>    <a href="#" class="button cdm-recycle-function" data-type="folder" data-id="'.$id.'" data-method="restore">'.__('Restore Folder', 'sp-client-document-manager').'</a></div>
				
		
			';
            echo '
		<div class="sp_su_project">	
	<strong>' . __("Recycled Date", "sp-client-document-manager") . ': </strong> ' . stripslashes($r[0]['recycle_date']) . '	
	</div>
		<div class="sp_su_project">	
	<strong>' . __("Automatic Removal Date", "sp-client-document-manager") . ': </strong>  ' . date("F j, Y", strtotime($r[0]['recycle_date'].'  + '.get_option('sp_cu_recycle_bin_retention', 30).' days')) . '	
	</div>
		<div class="sp_su_project">	
	<strong>' . __("Folder Name", "sp-client-document-manager") . ': </strong> ' . stripslashes($r[0]['name']) . '	
	</div>
	<div class="sp_su_project">	
	<strong>' . __("Folder Parent", "sp-client-document-manager") . ': </strong> ' . stripslashes(sp_cdm_get_project_name($r[0]['parent'])) . '	
	</div>';
    ...
    public function top_menu($menu)
    {

        $menu .='<li><a href="admin.php?page=sp-client-document-manager-recycle" >' . __("Recycle Bin", "sp-client-document-manager") . '</a></li>';
        return $menu ;
    }
    public function menu()
    {

        add_menu_page(__("Recycle Bin", "sp-client-document-manager"), __("Recycle Bin", "sp-client-document-manager"), 'sp_cdm', 'sp-client-document-manager-recycle', array(
        $this,
        'view_recycle_bin'
    ), 'dashicons-trash', 4);

    }
    public function view_recycle_bin()
    {
        ...
        echo '<div class="cdm-recycle-bin"><h2>'.__('Recycle Bin', 'sp-client-document-manager').'</h2><div class=cdm-recycle-bin-inner">   <div class="cdm-recycle-bin-menu">
			  <a href="#" class="button cdm-empty-recycle-bin">'.__('Empty Recycle Bin', 'sp-client-document-manager').'</a>    <a href="#" class="button cdm-restore-recycle-bin">'.__('Restore Everything', 'sp-client-document-manager').'</a>  <p><span>'.sprintf(__('Files are currently kept for %s days then removed automatically from the recycle bin. You can change the amount of days to keep files in advanced settings.', 'sp-client-document-manager'), $retention_date).'</span></p>
        ...
        echo '<h3>'.__('Recycled Folders', 'sp-client-document-manager').'</h3>
        ...
        if($r_folders) {
            ...
            echo '<p class="sp_cdm_error">'.__('No folders in recycle bin', 'sp-client-document-manager').'</p>';
        }

        echo '<h3>'.__('Recycled Files', 'sp-client-document-manager').'</h3>';
        if($r_files) {
            echo '<div class="cdm-recycled-items">';
            ...
            echo '<p class="sp_cdm_error">'.__('No files in recycle bin', 'sp-client-document-manager').'</p>';
        }
        echo '</div></div>';
    ...
    public function empty_reycle_bin_cron()
    {
    ...
    public function admin_settings()
    {
        ...
        echo '
				<tr><td colspan="2"><h2>'.__('Recycle Bin Settings', 'sp-client-document-manager').'</h2>
				<tr>

    <td width="300"><strong>'.__('Recycle bin rentention', 'sp-client-document-manager').'</strong><br><em>'.__('How many days to save the files before the system automatically deletes all the items in the recycle bin. Set to 0 to disable the recycle bin. Setting to 0 will automatically delete files and folders when you press the delete button.', 'sp-client-document-manager').'</em></td>
    ...
    public function restore_folder($project_id)
    {
        global $wpdb, $current_user;
        if (!is_user_logged_in()) {
            exit;
        ...
        cdm_recycle_folder($project_id, true);
    }


    public function preview_image($r)
    {
