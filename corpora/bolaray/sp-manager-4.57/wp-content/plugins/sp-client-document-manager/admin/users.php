...
class sp_cdm_admin_users
{
    ...
    public function delete_user_content_question($current_user, $userids)
    {
        global $wpdb;
	...
	<fieldset><p><legend><strong><?php _e('All document manager files and folders will be removed once this user is deleted.', 'sp-client-document-manager'); ?></strong></legend></p>
    	<input type="hidden" name="delete_option_cdm" value="delete" />
	<?php else: ?>
		<fieldset><p><legend><strong><?php _e('What should we do with the document manager content?', 'sp-client-document-manager'); ?></strong></legend></p>
		<ul style="list-style:none;">
			<li><label><input type="radio" id="delete_option0" name="delete_option_cdm" value="delete" checked />
			<?php _e('Delete all content.', 'sp-client-document-manager'); ?></label></li>
			<li><input type="radio" id="delete_option1" name="delete_option_cdm" value="reassign" />
			<?php echo '<label for="delete_option1">' . __('Move folders and files to:', 'sp-client-document-manager') . '</label> ';
