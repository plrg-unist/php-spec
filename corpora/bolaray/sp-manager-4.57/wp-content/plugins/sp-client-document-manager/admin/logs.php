...
public function view()
{
    ...
    echo '<h2>'.__("User Log", "sp-client-document-manager").'</h2>'.sp_client_upload_nav_menu().'';
...
public function submenu($content)
{

    if(current_user_can('sp_cdm_user_logs')) {
        $content .= '<li><a href="admin.php?page=sp-client-document-manager-user-logs" >' . __("User Logs", "sp-client-document-manager") . '</a></li>';
    }
    return $content;
...
function sp_cdm_write_log($string)
{

    global $sp_cdm_user_logs;

    $sp_cdm_user_logs->write($string);
}
