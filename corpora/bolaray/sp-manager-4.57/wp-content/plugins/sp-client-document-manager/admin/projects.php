...
<form action="admin.php?page=sp-client-document-manager-projects" method="post">' . wp_nonce_field('cdm_nonce', 'cdm_nonce', true, false) . '';
            ...
            if (cdm_var('parent') != '') {
                $parent = $wpdb->get_results("SELECT  * FROM " . $wpdb->prefix . "sp_cu_project where id = '" . intval(cdm_var('parent')) . "'  ", ARRAY_A);
  ...
  <tr>
    ...
    <th width="250">' . __("Name:", "sp-client-document-manager") . '</th>
    ...
    <th>' . __(sprintf("Parent %s", sp_cdm_folder_name()), "sp-client-document-manager") . '</th>

    <td>' . $spdm_sub_projects_new->project_dropdown_all('', $r[0]['id'], $parent) . '</td>
  ...
  <tr>
    ...
    <td>';
            wp_dropdown_users(array(
                'name' => 'uid',
                'selected' => $selected,
                'show_option_none' => __("Not Assigned", "sp-client-document-manager")
            ));
            echo '</td>
  ...
  <tr>
    ...
    <td><input type="submit" name="save-project" value="' . __("Save", "sp-client-document-manager") . '"></td>
...
';
        ...
        public function get_parent_count($id, $count = 0)
        {
        ...
        public function getChildren($id, $level = 0)
        {
...
<a href="javascript: cdm_download_project(' . $r[$i]['id'] . ',\'' . wp_create_nonce('my-action_' . $r[$i]['id']) . '\')" style="margin-right:15px" >' . __("Download Archive", "sp-client-document-manager") . '</a> ';

                    if ($r[$i]['parent'] == 0 or class_exists('spdm_sub_projects')) {

                        $html .= '<a href="admin.php?page=sp-client-document-manager-projects&function=add&parent=' . $r[$i]['id'] . '" style="margin-right:15px" >' . __(sprintf("Add Sub %s", sp_cdm_folder_name()), "sp-client-document-manager") . '</a> ';

                    }


                    $html .= '<a href="admin.php?page=sp-client-document-manager-projects&function=delete&id=' . $r[$i]['id'] . '" style="margin-right:15px" >' . __("Delete", "sp-client-document-manager") . '</a> 

<a href="admin.php?page=sp-client-document-manager-projects&function=edit&id=' . $r[$i]['id'] . '" >' . __("Modify", "sp-client-document-manager") . '</a></td>
<td></td>
        ...
        public function move_sub_folders($id, $uid)
        {
            global $wpdb;
        ...
        public function view()
        {
            ...
            if (cdm_var('function') == 'add' or cdm_var('function') == 'edit') {
                ...
                echo '
                                     ...
                                     <a href="admin.php?page=sp-client-document-manager-projects&function=add" class="button">' . __("Add", "sp-client-document-manager") . ' ' . sp_cdm_folder_name() . '</a> ';
...
<th style="width:60px"><strong>' . __("ID", "sp-client-document-manager") . '</strong></th>

<th><strong>' . __("Name", "sp-client-document-manager") . '</strong></th>

<th><strong>' . __("User", "sp-client-document-manager") . '</strong></th>

<th><strong>' . __("Action", "sp-client-document-manager") . '</strong></th>';
...
<td style="font-weight:bold;"><span class="sp-caret"></span> #' . intval($r[$i]['projectID']) . '</td>                

<td style="font-weight:bold;">' . esc_attr(stripslashes($r[$i]['projectName'])) . '</td>

<td style="font-weight:bold;">' . esc_attr($r[$i]['user_nicename']) . '</td>

<td style="font-weight:bold;">

<a href="javascript:cdm_download_project(' . intval($r[$i]['projectID']) . ',\'' . wp_create_nonce('cdm-download-archive') . '\');" style="margin-right:15px" >' . __("Download Archive", "sp-client-document-manager") . '</a>  ';


                    if ($r[$i]['parent'] == 0 or class_exists('spdm_sub_projects')) {

                        echo '<a href="admin.php?page=sp-client-document-manager-projects&function=add&parent=' . intval($r[$i]['projectID']) . '" style="margin-right:15px" >' . __(sprintf("Add Sub %s", esc_attr(sp_cdm_folder_name())), "sp-client-document-manager") . '</a> ';
                    ...
                    echo '<a href="admin.php?page=sp-client-document-manager-projects&function=delete&id=' . intval($r[$i]['projectID']) . '" style="margin-right:15px" >' . __("Delete", "sp-client-document-manager") . '</a> 

<a href="admin.php?page=sp-client-document-manager-projects&function=edit&id=' . intval($r[$i]['projectID']) . '" >' . __("Modify", "sp-client-document-manager") . '</a></td>

';
                    do_action('sp_cdm/admin/projects/list/loop', $r[$i]);
                    echo '</tr>';
                    echo '' . $this->getChildren(intval($r[$i]['projectID'])) . '</td>';
                } //$i = 0; $i < count($r); $i++
                echo '</tbody></table>';
            }
        }
