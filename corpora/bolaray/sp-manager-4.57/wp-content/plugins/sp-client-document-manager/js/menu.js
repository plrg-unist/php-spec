jQuery( document ).ready(function($) {
		...
		var form_data = $( this ).serializeArray();
			...
			jQuery.post(ajaxurl,form_data, function(response) {
					...
					var slug = $('[name="slug"]',form).val();
					var container = $('.sprm-addon-container-'+ slug);	
				
					
						console.log('.sp-rm-license-message-'+ slug);
					if(obj.result.license == 'valid'){
					$('.sp-rm-license-message-'+ slug).html('<p style="color:green">License Active! Expires: '+ obj.result.expires+'</p>');	
					$(".sprm-activate-button",container).val('Deactivate');
					$(".set_action",container).val('deactivate_license');
					}else{
					$('.sp-rm-license-message-'+ slug).html('<p style="color:red">License not activated.</p>');		
