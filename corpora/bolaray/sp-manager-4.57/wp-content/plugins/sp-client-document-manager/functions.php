...
function cdm_file_list_templates()
{
...
function cdm_verify_recpatcha($data)
{


    $args = array('body'=>array('token'=>$data['token'],'action'=>$data['action'],'secret'=>get_option('sp_cdm_recaptcha_secret_key')));
...
function cdm_is_folder_orphan($folder_id, $orphan=false)
{
    global $wpdb;

    $r_folder          = $wpdb->get_results($wpdb->prepare("SELECT *  FROM " . $wpdb->prefix . "sp_cu_project   where id = %d ", $folder_id), ARRAY_A);
    #echo '<pre>';
    #print_r($r_folder);
    #echo '</pre>';
    if($r_folder  == false) {
        ...
        if($r_folder[0]['parent'] != 0) {

            $orphan = cdm_is_folder_orphan($r_folder[0]['parent'], $orphan);
        } else {
            $orphan = false;
        }
...
function cdm_is_file_orphan($file_id)
{
    global $wpdb;
    $orphan = false;
    ...
    if($r[0]['pid'] == 0) {
        ...
        $orphan = cdm_is_folder_orphan($r[0]['pid']);
    }
...
function cdm_allow_iframes_filter($allowedposttags)
{
...
function cdm_move_file_to_user($from, $to, $file_id)
{
    ...
    if (file_exists($from_file)) {
        ...
        cdm_mkdir('' . SP_CDM_UPLOADS_DIR . '' . $to . '/', 0777);
...
function cdm_parent_exists($file_id)
{
    ...
    if ($file_id != 0 && $file_id != '') {
        $query = $wpdb->prepare("SELECT *  FROM " . $wpdb->prefix . "sp_cu   where id= %d ", intval($file_id));
        $r = $wpdb->get_results($query, ARRAY_A);

        if ($r == false) {
            $lost_children = $wpdb->get_results($wpdb->prepare("SELECT *  FROM " . $wpdb->prefix . "sp_cu   where parent = %d ", intval($file_id)), ARRAY_A);
            $ok = false;
...
if (!function_exists('sp_client_upload_settings')) {
    ...
    function cdm_user_can_delete($uid)
    {
        ...
        if (
            ...
            ((
                ($current_user->ID ==  $uid or cdmFindLockedGroup($current_user->ID, $uid) == true) &&
        get_option('sp_cu_user_delete_disable') != 1
    ...
    function cdm_user_can_add($uid)
    {
        ...
        if (get_option('sp_cu_user_uploads_disable') != 1 and(
            (get_option('sp_cdm_groups_addon_global_add_roles_' . sp_cdm_get_current_user_role_name() . '') == ''
              or get_option('sp_cdm_groups_addon_global_add_roles_' . sp_cdm_get_current_user_role_name() . '') == 1)
        )) {
            return true;
    ...
    function cdm_mkdir($dir, $permission)
    {
...
';
	...
	<ul id="menu1" style="margin-top:20px;margin-bottom:10px;">';
                if (current_user_can('sp_cdm')) {
                    $content .= '<li><a href="admin.php?page=sp-client-document-manager" >' . __("Home", "sp-client-document-manager") . '</a></li>';
                }
                if (current_user_can('sp_cdm_settings')) {
                    $content .= '<li><a href="admin.php?page=sp-client-document-manager-settings" >' . __("Settings", "sp-client-document-manager") . '</a><ul>';
                    $content .= '<li><a href="admin.php?page=sp-client-document-manager-settings" >' . __("Global Settings", "sp-client-document-manager") . '</a></li>';
                    if (current_user_can('sp_cdm_vendors')) {
                        $content .= '<li><a href="admin.php?page=sp-client-document-manager-vendors" >' . __("Vendors", "sp-client-document-manager") . '</a></li>';
                    }
                    ...
                    if (cdm_is_premium_active() == 1) {
                        if (current_user_can('sp_cdm_groups')) {
                            $content .= '<li><a href="admin.php?page=sp-client-document-manager-groups" >' . __("Share Spaces", "sp-client-document-manager") . '</a></li>';
                        }
                        if (current_user_can('sp_cdm_forms')) {
                            $content .= '<li><a href="admin.php?page=sp-client-document-manager-forms">' . __("Forms", "sp-client-document-manager") . '</a></li>';
                        }
                        if (current_user_can('sp_cdm_categories')) {
                            $content .= '<li><a href="admin.php?page=sp-client-document-manager-categories" >' . __("Categories", "sp-client-document-manager") . '</a></li>';
                        }
                    }
                    $extra_menus = '';
                ...
                if (current_user_can('sp_cdm_uploader')) {
                    $content .= '<li><a href="admin.php?page=sp-client-document-manager-fileview" >' . __("User Files / Uploader", "sp-client-document-manager") . '</a></li>

			';
                }
                if (current_user_can('sp_cdm_help')) {
                    $content .= '	
	<li><a href="admin.php?page=sp-client-document-manager-help" >' . __("Instructions", "sp-client-document-manager") . '</a></li>';
                }
            ...
            if (current_user_can('sp_cdm_help')) {
                ...
                $content .= '<div style="text-align:right"><strong style="margin-right:10px">' . __("Community Version", "sp-client-document-manager") . ':</strong> ' .esc_attr(get_option('sp_client_upload')) . '';
                if (cdm_is_premium_active() == 1) {
                    $content .= ' <strong style="margin-left:50px;margin-right:10px;">' . __("Premium Version", "sp-client-document-manager") . ':</strong> ' . esc_attr(get_option('sp_client_upload_premium')) . '';
...
<a href="http://smartypantsplugins.com/sp-client-document-manager/" target="_blank" class="button">Click here to upgrade! </a> <a style="margin-left:10px" href="http://www.youtube.com/watch?feature=player_embedded&v=m6szdA3r-1Q" target="_blank" class="button">View the youtube video</a> <a style="margin-left:10px" href="http://smartypantsplugins.com/donate/" target="_blank" class="button">Click here to donate</a> <a href="admin.php?page=sp-client-document-manager&sphidemessage=1"  class="button" style="margin-left:10px">Click here to ignore us!</a></p>
            ...
            if (get_option('sp_cu_uploads_page') != '') {
                ...
                $url = esc_attr(get_permalink(get_option('sp_cu_uploads_page')));
                $page_id = esc_attr(get_option('sp_cu_uploads_page'));
            ...
            if ($page_id == "" && get_option('cdm_ignore_shortcode') != 1) {
                ...
                if (cdm_var('page-name-cdm')) {
                    ...
                    // Create post object
                    $my_post = array(
                      'post_title' => esc_attr(cdm_var('page-name-cdm')),
                    ...
                    $content .= '<div style="margin:10px;font-size:1.3em" class="sp_cdm_success"><strong>' . esc_attr(cdm_var('page-name-cdm')) . '</strong> Page Created! <a href="' . get_page_link($post) . '" target="_blank">Click here to preview the page</a></div>';
                } else {
    ...
    if (!function_exists('sp_client_upload_admin')) {
        function sp_client_upload_admin()
        {
            ...
            $search = '';
            $download_user = '';
            $user_id = esc_attr(cdm_var('user_id'));
            $file = esc_attr(cdm_var('dlg-delete-file'));
            $search_vars = array();
            if ($file != "") {

                cdm_recycle_file(esc_attr(cdm_var('dlg-delete-file')));

            }
            if (cdm_var('cdm_limit')) {
                update_option('cdm_admin_listings_limit', esc_attr(cdm_var('cdm_limit')));
            }
            $limit = get_option('cdm_admin_listings_limit', 15);

            if (cdm_var('from') != '') {
                $search .= " AND date > %s ";
                $search_vars[] = date("Y-m-d", strtotime(esc_attr(cdm_var('from')))) ;
            }
            if (cdm_var('to') != '') {
                $search .= " AND date %s ";
                $search_vars[] =  date("Y-m-d", strtotime(esc_attr(cdm_var('to'))));
            }

            if (cdm_var('s') != '') {
                $search .= " AND  (name like %s or file like %s or tags like %s  )";

                $search_vars[] = "%" . cdm_var('s') . "%";
                $search_vars[] = "%" . cdm_var('s') . "%";
                $search_vars[] = "%" . cdm_var('s') . "%";
            }

            $search = apply_filters('sp_cdm/admin/search_files/query', $search);
            $search_vars= apply_filters('sp_cdm/admin/search_files/query_vars', $search_vars);
            if ($user_id != '' && $user_id != '-1') {

                echo '<h2>' . __("User Uploads", "sp-client-document-manager") . '</h2><a name="downloads"></a>';
                $r = $wpdb->get_results($wpdb->prepare("SELECT *  FROM " . $wpdb->prefix . "sp_cu   where uid = %d  and parent = 0 and recycle = 0 order by date desc", $user_id), ARRAY_A);
                $delete_page = 'user-edit.php?user_id=' . $user_id . '';

            } else {

                $search_vars[] = $limit;
                $r = $wpdb->get_results($wpdb->prepare("SELECT *  FROM " . $wpdb->prefix . "sp_cu   where  parent = 0 " . $search . " and recycle = 0 order by id desc LIMIT %d", $search_vars), ARRAY_A);
...
<strong>Search Term</strong><br> <input type="text" name="s" value="' . esc_attr(cdm_var('s')) . '">
</td>
<td >
<strong>User</strong><br> 
' . wp_dropdown_users(array( 'name' => 'user_id', 'show_option_none' => 'Choose a user', 'selected' => intval(cdm_var('user_id')), 'echo' => false )) . '
</td>
' . apply_filters('sp_cdm/admin/search_files/fields', '') . '
<td style="width:70px">
<strong>Show</strong> <br><input style="width:50px"type="text" name="cdm_limit" value="' . esc_attr(get_option('cdm_admin_listings_limit', 15)). '">  </td><td style="width:170px"><label for="from"><strong>From</strong></label>
<br><input type="text" id="from" name="from" value="' . esc_attr(cdm_var('from')) . '"></td><td style="width:170px">
<label for="to"><strong>to</strong></label><br>
<input type="text" id="to" name="to"  value="' . esc_attr(cdm_var('to')) . '"></td><td><input style="margin-top:20px;" type="submit" value="Show" name="limit">
</td></tr></table>
</form>';
            }
            if ($r == false) {
                $html .= '<p style="color:red">' . __("No Uploads Exist!", "sp-client-document-manager") . '</p>';
            } else {
...
' . $download_user . '';
  ...
  <table class="wp-list-table widefat fixed posts" cellspacing="0">
	...
	<th style="width:30px">' . __("ID", "sp-client-document-manager") . '</th>	

<th style="width:80px">' . __("Thumbnail", "sp-client-document-manager") . '</th>	

<th>' . __("File Name", "sp-client-document-manager") . '</th>

<th>' . __("User", "sp-client-document-manager") . '</th>

<th>' . __("Date", "sp-client-document-manager") . '</th>

<th>' . __("Download", "sp-client-document-manager") . '</th>

<th>' . __("Email", "sp-client-document-manager") . '</th>';
 ...
 <td>' . $img . '</td>
    ...
    <td ><strong>' . stripslashes($name) . '</strong>';
                    if (cdm_is_premium_active() == 1) {
                        ...
                        $html .= '<br><em>' . __("Notes: ", "sp-client-document-manager") . ' ' . stripslashes($r[ $i ][ 'notes' ]) . '</em>';
                    }
                    if ($r[ $i ][ 'tags' ] != "") {
                        $html .= '<br><strong>' . __("Tags ", "sp-client-document-manager") . '</strong><em>: ' . $r[ $i ][ 'tags' ] . '</em>';
                    }
                    $html .= '
    ...
    <td><a style="margin-right:15px" href="javascript:cdmViewFile(' . $r[ $i ][ 'id' ] . ')" >' . __("View", "sp-client-document-manager") . '</a> <a href="' . $delete_page . '&dlg-delete-file=' . $r[ $i ][ 'id' ] . '#downloads">' . __("Delete", "sp-client-document-manager") . '</a> </td>
...
<td><input type="checkbox" name="vendor_email[]" value="' . $r[ $i ][ 'id' ] . '"></td>';
  ...
  ';
			...
			<div id="updateme"></div>
				...
				' . __("Choose  the files you want to send above, type a message and choose a vendor then click submit:", "sp-client-document-manager") . '  <select name="vendor">
                ...
                $html .= '</select> ' . __("Message:", "sp-client-document-manager") . ' <input type="text" name="vendor-message"> <select name="vendor_attach"><option value="1">' . __("Attach to email:", "sp-client-document-manager") . ' </option><option value="0">' . __("Send links to files", "sp-client-document-manager") . ' </option><option value="3">' . __("Attach and link to to files", "sp-client-document-manager") . ' </option></select> <input type="submit" name="submit-vendor" value="' . __("Email vendor files!", "sp-client-document-manager") . '" onclick="sp_client_upload_email_vendor();return false;"> 
            ...
            if ($user_id != "") {
                echo cdm_esc_html($html);
            } else {

                return cdm_esc_html($html);
            }
        }
    }
}
