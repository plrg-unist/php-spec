...
add_action('plugins_loaded', array($sp_cdm_installer,'protect_dirs'));
...
{
    public function protect_dirs()
    {

        if(get_option('cdm_protect_dirs') != '1') {
            $this->readDirs(SP_CDM_UPLOADS_DIR);
            update_option('cdm_protect_dirs', '1');
        }
    ...
    public function readDirs($path)
    {
        ...
        while($item = readdir($dirHandle)) {
            ...
            if(is_dir($newPath) && $item != '.' && $item != '..') {
                cdm_mkdir($newPath, 0777);
                $this->readDirs($newPath);
            }
        }
    }

    public function sp_client_upload_upgrader()
    {
        global $wpdb;
        ...
        public function admin_bar($admin_bar)
        {
            ...
            if(current_user_can('sp_cdm')) {
                ...
                'title' => '<span class="ab-icon"></span><span class="ab-label">'.__('Documents Dashboard', 'sp-client-document-manager').'</span>',
                'href'  => get_admin_url().'admin.php?page=sp-client-document-manager',
                'meta'  => array(
                    'title' => __('Document Dashboard', "sp-client-document-manager"),
                ),
                ));
                ...
                if(current_user_can('sp_cdm_settings')) {
                    ...
                    'meta'  => array(
                        'title' => __('Settings', "sp-client-document-manager"),
                    ),
                    ));
                }
                if(current_user_can('sp_cdm_uploader')) {
                    ...
                    'title' => '<span class="ab-icon"></span><span class="ab-label">'.__('Admin Uploader', 'sp-client-document-manager').'</span>',
                    'href'  => get_admin_url().'admin.php?page=sp-client-document-manager-fileview',
                    'meta'  => array(
                        'title' => __('Document Uploader', "sp-client-document-manager"),
                    ),
                    ));
                }
            }
    ...
    public function sp_cdm_check_admin_caps()
    {
        global $current_user;
    ...
    public function sp_cdm_premium_upgrades()
    {
        ...
        if($wpdb->get_var("show tables like '$table_name'") != $table_name) {
            $sql_meta = "CREATE TABLE IF NOT EXISTS `".$wpdb->prefix."sp_cu_meta` (
    ...
    public function install()
    {
        ...
        cdm_mkdir($cdm_upload_dir['basedir'], 0777);
    ...
    public function check_caps()
    {
        ...
        public function db_tables()
        {
            ...
            $db_tables[$wpdb->prefix ."sp_cu_cats"] =
            ...
            $db_tables[$wpdb->prefix ."sp_cu_forms"] =
            ...
            $db_tables[$wpdb->prefix ."sp_cu_form_entries"] =
            ...
            $db_tables[$wpdb->prefix ."sp_cu_groups"] =
            ...
            $db_tables[$wpdb->prefix ."sp_cu_groups_assign"] =
            ...
            $db_tables[$wpdb->prefix ."sp_cu_project"] =
