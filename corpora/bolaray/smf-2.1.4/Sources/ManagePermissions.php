...
function SetQuickGroups()
{
	...
	if (!empty($_POST['predefined']))
	{
		...
		{
			if (!empty($_REQUEST['pid']))
				setPermissionLevel($_POST['predefined'], 0, $_REQUEST['pid']);
			else
				setPermissionLevel($_POST['predefined'], 0);
		}
	}
	...
	elseif ($_POST['copy_from'] != 'empty')
	{
		...
		$_POST['group'] = array_diff(0, array($_POST['copy_from']));
