...
CASES = json.loads((ROOT / 'tests/semantics/builtin_write_cases.json').read_text())
...
def main():
    ...
    files = [*modules, runner, adapter_path, types.PHP, Path(__file__),
             ROOT / 'tests/semantics/builtin_write_cases.json',
             ROOT / 'frontend/worker.php', ROOT / '.tools/php-file.so']
    before = {str(p.relative_to(ROOT)): digest(p) for p in files}
    out = Path(tempfile.mkdtemp(prefix='builtin-write-compiler-', dir=ROOT / '.tools'))
    ...
    (ROOT / 'coverage/semantics/builtin-write-compiler.json').write_text(json.dumps(report, indent=2) + '\n')
