#!/usr/bin/env python3
"""Main-script persistent cells through global rebinding, unset and cached loop initialization."""
...
SELECTION = ['main-static-stored-global-alias', 'main-static-dynamic-loop-unset']
...
def main():
    ...
    out = Path(tempfile.mkdtemp(prefix='main-statics-state-', dir=ROOT / '.tools'))
    ...
    try:
        for name in SELECTION:
            directory = out / name
            directory.mkdir()
            source = main_statics.CASES[name]
            ...
            stored = name == 'main-static-stored-global-alias'
            stages = [('first', 0 if stored else 1)] + ([] if stored else [('later', 2)])
            ...
            checks += ['S_first.STATICS = eps', 'S_first.CURRENT = eps', 'S_first.GLOBALTABLE = eps',
                       'S_bound = $drive(S, $(n_first + 1))', '|S_bound.STATICS| = 1',
                       'pstaticcell = S_bound.STATICS[0]', 'n_static = pstaticcell.CELL',
                       '$main_static_origin(S_bound, pstaticcell.ORIGIN)',
            ...
            if stored:
                checks += ['$lookup(S_first.ENV, [120]) = (n_old)', '$lookup(S_first.ENV, [97]) = (n_old)',
                           'S_first.STORE[n_old] = DEFINED (PINT 4)', 'n_old =/= n_static',
                           '$lookup(S_bound.ENV, [97]) = (n_old)', 'S_bound.STORE[n_old] = DEFINED (PINT 4)',
                           'S_bound.STORE[n_static] = DEFINED (PINT 7)',
                           '$lookup(S_out.ENV, [120]) = (n_static)', '$lookup(S_out.ENV, [97]) = (n_old)',
                           'S_out.STORE[n_static] = DEFINED (PINT 8)', 'S_out.STORE[n_old] = DEFINED (PINT 4)',
                           '$heap_owners($heap_graph(S_out), HCELL n_static) = 2',
                           '$heap_owners($heap_graph(S_out), HCELL n_old) = 1']
            else:
                checks += ['S_first.RESULT = KNOWN (PINT 1)', 'S_bound.STORE[n_static] = DEFINED (PINT 1)',
                           'S_later.STATICS = S_bound.STATICS', '$lookup(S_later.ENV, [120]) = eps',
                           'S_later.STORE[n_static] = DEFINED (PINT 2)',
                           '$heap_owners($heap_graph(S_later), HCELL n_static) = 1',
                           'S_rebound = $drive(S, $(n_later + 1))',
                           'S_rebound.STATICS = S_bound.STATICS', '$lookup(S_rebound.ENV, [120]) = (n_static)',
                           'S_rebound.STORE[n_static] = DEFINED (PINT 2)',
                           '$lookup(S_out.ENV, [120]) = eps', 'S_out.STORE[n_static] = DEFINED (PINT 3)',
    ...
    (ROOT / 'coverage/semantics/main-statics-state.json').write_text(json.dumps(report, indent=2))
    return True
