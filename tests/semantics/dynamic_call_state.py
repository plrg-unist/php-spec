#!/usr/bin/env python3
"""Suspended dynamic lookup, callee frames and reference-return owners."""
...
SELECTION = ['dynamic-callee-before-arguments', 'dynamic-reference-return']

PREFIX = d.PREFIX + f.PREFIX + r'''
dec $dynamic_state_stage(pstate, nat) : bool
def $dynamic_state_stage(S, 0) = true -- if S.TODO = (CALL_DYNAMIC porigin z_init z_call) :: ptask*
def $dynamic_state_stage(S, 1) = true
  -- if S.TODO = (CALL_ARGS porigin_function phpType7* 0 poperand* porigin? z) :: ptask*
  -- if $dynamic_site(S, porigin?)
def $dynamic_state_stage(S, 2) = true -- if S.TODO = ACQUIRE_CALL :: ptask*
def $dynamic_state_stage(S, 3) = true
  -- if S.CURRENT = (pcallcontext)
  -- if pcallcontext.NAME = [110]
def $dynamic_state_stage(S, n) = false -- otherwise
dec $dynamic_state_cut(pstate, nat, nat, nat) : nat
def $dynamic_state_cut(S, n_kind, n, n_left) = n -- if $dynamic_state_stage(S, n_kind)
def $dynamic_state_cut(S, n_kind, n, n_left) = $dynamic_state_cut($drive_steps(S[.COMPLETION = NORMAL], 1), n_kind, $(n + 1), n_rest)
  -- if ~$dynamic_state_stage(S, n_kind)
  -- if $(n_left > 0)
  -- if n_rest = $(n_left - 1)
...
def main():
    ...
    out = Path(tempfile.mkdtemp(prefix='dynamic-call-state-', dir=ROOT / '.tools'))
    ...
    try:
        for name in SELECTION:
            directory = out / name
            directory.mkdir()
            source = dynamic_call.CASES[name]
            ...
            reference = name == 'dynamic-reference-return'
            stages = [('lookup', 0), ('selected', 1)]
            if reference:
                stages.append(('acquired', 2))
            else:
                stages.insert(0, ('callee', 3))
            for stage, kind in stages:
                checks += [f'n_{stage} = $dynamic_state_cut(S, {kind}, 0, 512)',
                           f'S_{stage} = $drive(S, n_{stage})', f'$dynamic_state_stage(S_{stage}, {kind})']
            checks += ['S_lookup.TODO = (CALL_DYNAMIC porigin z_init z_call) :: ptask_lookup*',
                       'S_selected.TODO = (CALL_ARGS porigin_function phpType7* 0 poperand* porigin_selected? z_selected) :: ptask_selected*',
                       'poperand* = eps', 'porigin_selected? = (porigin)', 'z_selected = z_call',
                       'S_selected.RESULT = KNOWN PNULL', 'S_selected.BASE = BASE_VALUE (KNOWN PNULL)',
                       '$function_at(S_selected.FUNCTIONS, porigin_function) = (pfunction)',
                       '$callable_at(S_selected.CALLABLES, $ptlc(pfunction.NAME)) = (porigin_function)']
            if reference:
                checks += ['pfunction.NAME = [114]', 'n_name* = [110]', 'S_lookup.RESULT = VARIABLE n_name* z_name',
                           'S_acquired.RESULT = REFERENCE n_returned', '$lookup(S_acquired.ENV, [97]) = (n_returned)',
                           'S_acquired.STORE[n_returned] = DEFINED (PINT 1)',
                           '$($heap_owners($heap_graph(S_acquired), HCELL n_returned) >= 2)',
                           '$lookup(S_out.ENV, [97]) = (n_a)', '$lookup(S_out.ENV, [120]) = (n_x)',
                           'n_a = n_x', 'n_a <- S_out.REFCELLS', 'S_out.STORE[n_a] = DEFINED (PINT 3)',
                           '$($heap_owners($heap_graph(S_out), HCELL n_a) >= 2)']
            else:
                checks += ['pfunction.NAME = [102]', 'n_f* = [102]', 'S_lookup.RESULT = KNOWN (PSTRING n_f*)',
                           '$outputs(S_lookup.EVENTS) = [67]', '$outputs(S_selected.EVENTS) = [67]',
                           'S_lookup.FRAMES = eps', 'S_lookup.CURRENT = eps',
                           'S_callee.FRAMES =/= eps', '$call_descriptors_valid(S_callee)',
                           'S_out.REFCELLS = eps']
    ...
    (ROOT / 'coverage/semantics/dynamic-call-state.json').write_text(json.dumps(report, indent=2))
    return True
