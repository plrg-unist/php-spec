#!/usr/bin/env python3
"""Strict declaration compiler phases and lexical code projections."""
import base64
import hashlib
import json
...
CASES = json.loads((ROOT / 'tests/semantics/strict_declaration_cases.json').read_text())


PENDING = {'other-directive': 'other declare directive compilation',
           'mixed-directive': 'other declare directive compilation'}


def contexts(name, source):
    strict = 'false' if name == 'zero' else 'true'
    checks = [f'P.ENV.STRICT = {strict}', f'$ppfunction_code(P).STRICT = {strict}']
    if 'function f' in source:
        checks += ['P.FUNCTIONS = [pfunction]',
                   f'pfunction.ENV.STRICT = {strict}',
                   f'pfunction.CODE.STRICT = {strict}']
    return checks
...
def main():
    ...
    files = [*modules, runner, adapter_path, types.PHP, Path(__file__),
             ROOT / 'tests/semantics/strict_declaration_cases.json',
    ...
    out = Path(tempfile.mkdtemp(prefix='strict-declaration-compiler-', dir=ROOT / '.tools'))
    ...
    try:
        for index, (name, source) in enumerate(CASES.items()):
            ...
            if name in PENDING:
                ...
                checks = contexts(name, source)
                context_count += len(checks)
    ...
    report = {'scope': 'Strict declaration compiler phases and lexical code projections; runtime directive/source guards are a separate gate.',
              'result': 'pass', 'compared': len(CASES)-len(PENDING), 'pending': PENDING, 'context_assertions':context_count, 'cases': records, 'inputs': before, 'raw': str(out.relative_to(ROOT)),
              'fixture_sha256': digest(fixture)}
    (ROOT / 'coverage/semantics/strict-declaration-compiler.json').write_text(json.dumps(report, indent=2) + '\n')
