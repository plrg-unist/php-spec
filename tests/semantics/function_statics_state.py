#!/usr/bin/env python3
"""Persistent static cells through recursive initialization and local reference replacement."""
...
SELECTION = ['static-reentry-inner-wins', 'static-binding-releases-prior-reference']

PREFIX = d.PREFIX + f.PREFIX + r'''
dec $static_state_stage(pstate, nat) : bool
def $static_state_stage(S, 0) = true
  -- if S.TODO = (STATIC_BIND porigin) :: ptask*
  -- if S.STATICS = eps
def $static_state_stage(S, 1) = true
  -- if S.TODO = (STATIC_BIND porigin) :: ptask*
  -- if S.STATICS =/= eps
def $static_state_stage(S, 2) = true
  -- if S.TODO = (STATIC_INIT porigin) :: ptask*
  -- if S.STATICS =/= eps
def $static_state_stage(S, n) = false -- otherwise
dec $static_state_cut(pstate, nat, nat, nat) : nat
def $static_state_cut(S, n_kind, n, n_left) = n -- if $static_state_stage(S, n_kind)
def $static_state_cut(S, n_kind, n, n_left) = $static_state_cut($drive_steps(S[.COMPLETION = NORMAL], 1), n_kind, $(n + 1), n_rest)
  -- if ~$static_state_stage(S, n_kind)
  -- if $(n_left > 0)
  -- if n_rest = $(n_left - 1)
...
def main():
    ...
    out = Path(tempfile.mkdtemp(prefix='function-statics-state-', dir=ROOT / '.tools'))
    ...
    try:
        for name in SELECTION:
            directory = out / name
            directory.mkdir()
            source = function_statics.CASES[name]
            ...
            recursive = name == 'static-reentry-inner-wins'
            stages = [('first', 0), ('later', 1 if recursive else 2)]
            for stage, kind in stages:
                checks += [f'n_{stage} = $static_state_cut(S, {kind}, 0, 512)',
                           f'S_{stage} = $drive(S, n_{stage})', f'$static_state_stage(S_{stage}, {kind})']
            checks += ['S_first.TODO = (STATIC_BIND porigin) :: ptask_first*',
                       'S_first.STATICS = eps', 'S_bound = $drive(S, $(n_first + 1))',
                       '|S_bound.STATICS| = 1', 'pstaticcell = S_bound.STATICS[0]',
                       'pstaticcell.ORIGIN = porigin', 'n_static = pstaticcell.CELL',
                       'n_static <- S_bound.REFCELLS',
                       '$lookup(S_bound.ENV, [120]) = (n_static)',
                       'S_bound.RESULT = KNOWN PNULL', 'S_bound.BASE = BASE_VALUE (KNOWN PNULL)',
                       '$($heap_owners($heap_graph(S_bound), HCELL n_static) >= 2)',
                       '$static_cells_valid(S_bound, S_bound.STATICS)',
                       'S_out.STATICS = S_bound.STATICS',
                       '$heap_owners($heap_graph(S_out), HCELL n_static) = 1']
            if recursive:
                checks += ['S_first.RESULT = KNOWN (PINT 1)', 'S_bound.STORE[n_static] = DEFINED (PINT 1)',
                           'S_later.TODO = (STATIC_BIND porigin) :: ptask_later*',
                           'S_later.RESULT = KNOWN (PINT 11)', 'S_later.STATICS = S_bound.STATICS',
                           'S_later.STORE[n_static] = DEFINED (PINT 1)',
                           'S_rebound = $drive(S, $(n_later + 1))',
                           'S_rebound.STATICS = S_bound.STATICS',
                           '$lookup(S_rebound.ENV, [120]) = (n_static)',
                           'S_rebound.RESULT = KNOWN PNULL',
                           'S_rebound.STORE[n_static] = DEFINED (PINT 1)',
                           'S_out.STORE[n_static] = DEFINED (PINT 1)']
            else:
                checks += ['S_first.RESULT = KNOWN (PINT 2)',
                           '$lookup(S_first.ENV, [120]) = (n_y)', '$lookup(S_first.ENV, [121]) = (n_y)',
                           'S_first.STORE[n_y] = DEFINED (PINT 1)', 'n_y =/= n_static',
                           'S_bound.STORE[n_static] = DEFINED (PINT 2)',
                           'S_later.TODO = (STATIC_INIT porigin) :: ptask_later*',
                           'S_later.STATICS = S_bound.STATICS',
                           '$lookup(S_later.ENV, [120]) = (n_y)', '$lookup(S_later.ENV, [121]) = (n_y)',
                           'S_later.STORE[n_static] = DEFINED (PINT 3)',
                           'S_rebound = $drive(S, $(n_later + 1))',
                           '$lookup(S_rebound.ENV, [120]) = (n_static)',
                           'S_rebound.STORE[n_static] = DEFINED (PINT 3)',
                           'S_out.STORE[n_static] = DEFINED (PINT 4)',
                           '$lookup(S_out.ENV, [121]) = (n_y)', 'S_out.STORE[n_y] = DEFINED (PINT 1)']
    ...
    (ROOT / 'coverage/semantics/function-statics-state.json').write_text(json.dumps(report, indent=2))
    return True
