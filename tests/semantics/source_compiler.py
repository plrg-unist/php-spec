...
def main():
    ...
    try:
        with tempfile.TemporaryDirectory(dir=ROOT/'.tools') as tmp:
            ...
            for source,message in [(b'<?php namespace N {echo 1;} namespace {echo 2;} __halt_compiler();','ordinary statement compilation')]:
                i=len(fixtures);parsed=f.request({'op':'parse','source':base64.b64encode(source).decode()});assert parsed['accepted'],parsed
                checked=a.request({'op':'check','ast':parsed['ast'],'fixture':True})
            ...
            print(len(ACCESS_CASES),'access sources;',sum(len(probes) for source,probes in ACCESS_CASES),'access paths;',len(SOURCE_CASES),'compiler lint comparisons;',len(LINES),'emission-line observations; 1 Unsupported context; 7 metadata boundaries; constant export merge checks passed')
    finally:f.close();a.close()
    assert before==fingerprint(),'watched inputs changed'
    report={'scope':'ordered compiler work, access and operand descriptors; source execution tested separately', 'profile':types.PROFILE,'oracle_identity':identity,'fingerprint':before,'baseline_cases':len(runtime_cases.CASES),'targeted_lint_cases':len(CASES),'byte_diagnostic_checks':['invalid UTF-8 roundtrip','Unicode separators are not output line breaks'],'access_sources':len(ACCESS_CASES),'access_paths':sum(len(probes) for source,probes in ACCESS_CASES),'emission_line_cases':len(LINES),'unsupported_contexts':1,'constant_export_checks':['matching duplicate','conflicting scalar','conflicting array ID','missing operand line','missing fact line','incomplete compilation'],'metadata_boundaries':[None,'0','-1'],'break_terminator_boundaries':[None,'0','-1','99'],'cases':records}
