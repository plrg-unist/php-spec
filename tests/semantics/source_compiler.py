...
def main():
    ...
    try:
        with tempfile.TemporaryDirectory(dir=ROOT/'.tools') as tmp:
            ...
            print(len(ACCESS_CASES),'access sources;',sum(len(probes) for source,probes in ACCESS_CASES),'access paths;',len(SOURCE_CASES),'compiler lint comparisons;',len(LINES),'emission-line observations; 10 Unsupported contexts; 3 metadata boundaries; constant export merge checks passed')
    finally:f.close();a.close()
    assert before==fingerprint(),'watched inputs changed'
    report={'scope':'ordered compiler work, access and operand descriptors; no source runtime integration', 'profile':types.PROFILE,'oracle_identity':identity,'fingerprint':before,'baseline_cases':len(runtime_cases.CASES),'targeted_lint_cases':len(CASES),'byte_diagnostic_checks':['invalid UTF-8 roundtrip','Unicode separators are not output line breaks'],'access_sources':len(ACCESS_CASES),'access_paths':sum(len(probes) for source,probes in ACCESS_CASES),'emission_line_cases':len(LINES),'unsupported_contexts':10,'constant_export_checks':['matching duplicate','conflicting scalar','conflicting array ID','missing operand line','missing fact line','incomplete compilation'],'metadata_boundaries':[None,'0','-1'],'cases':records}
