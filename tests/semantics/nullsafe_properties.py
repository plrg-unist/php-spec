#!/usr/bin/env python3
"""Replay retained nullsafe property sources against the checked CLI."""
...
CASES = json.loads((ROOT / 'tests/semantics/nullsafe_properties_cases.json').read_text())
...
def run(match):
    ...
    assert selected, 'no nullsafe source controls selected'
    ...
    watched = [*modules, ROOT / 'spec/semantics/modules.json', Path(__file__),
               ROOT / 'tests/semantics/nullsafe_properties_cases.json',
               ROOT / 'bin/php-semantics', ROOT / 'tests/semantics/profile.json']
    direct = {str(path.relative_to(ROOT)): sha(path) for path in watched}
    out = Path(tempfile.mkdtemp(prefix='nullsafe-properties-', dir=ROOT / '.tools'))
    ...
    for row in selected:
        ...
        passed = result.returncode == 0 and actual.get('status') == row['status']
