...
SELECTION = ['literal-two-defaults','numeric-warning-repeated','warning-empty-array-result','body-error-after-default']
...
def main():
    ...
    out = Path(tempfile.mkdtemp(prefix='default-parameter-state-', dir=ROOT / '.tools'))
    ...
    try:
        for name in SELECTION:
            directory = out / name
            directory.mkdir()
            source = default_parameters.CASES[name]
            ...
            completion = 'NORMAL'
            if name == 'body-error-after-default':
                completion = 'THROWN "DivisionByZeroError" ([68,105,118,105,115,105,111,110,32,98,121,32,122,101,114,111]) 1'
            ...
            if name in ('literal-two-defaults', 'body-error-after-default'):
                checks += ['S_out.DEFAULTCACHE = eps']
            if name == 'body-error-after-default':
                checks += ['S_out.ALLOCATIONS = S.ALLOCATIONS']
            if name == 'numeric-warning-repeated':
                checks += ['S_out.DEFAULTCACHE = [pdefaultcache]', 'pdefaultcache.VALUE = PINT 3', 'pdefaultcache.CLASS = PVSCALAR']
            if name == 'warning-empty-array-result':
                checks += ['S_out.DEFAULTCACHE = [pdefaultcache]', 'pdefaultcache.VALUE = PARRAY n_cache', 'pdefaultcache.CLASS = PVARRAY true eps', 'S_out.ARRAYS[n_cache].ITEMS = eps']
    ...
    (ROOT / 'coverage/semantics/default-parameter-state.json').write_text(json.dumps(report, indent=2))
    return True
