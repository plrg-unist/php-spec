#!/usr/bin/env python3
"""Unimplemented clone dependencies remain Unsupported, not PHP agreement."""
import hashlib
import json
...
CASES = {
    'clone-callback': '<?php class C{function __clone(){echo "C";}}$a=clone new C;',
    'readonly': '<?php class C{public readonly int $x;}$a=clone new C;',
    'property-hook': '<?php class C{public int $x{get=>1;}}$a=clone new C;',
    'destructor': '<?php class C{function __destruct(){echo "D";}}$a=clone new C;',
}


def main():
    out = Path(tempfile.mkdtemp(prefix='clone-controls-', dir=ROOT / '.tools'))
    ...
    print(out, '4 Unsupported controls pass')
