#!/usr/bin/env python3
"""Private positional reference parameter phase and fetch descriptors, separate from execution."""
import base64
import hashlib
import json
...
CASES = json.loads((ROOT / 'tests/semantics/function_reference_cases.json').read_text())


# Original checked source occurrences, independently inspected in retained ASTs.
ARG1 = '[PCINDEX 1, PCFIELD 0, PCFIELD 1, PCINDEX 0, PCFIELD 1]'
ARG0 = '[PCINDEX 0, PCFIELD 0, PCFIELD 1, PCINDEX 0, PCFIELD 1]'
ARG2 = '[PCINDEX 2, PCFIELD 0, PCFIELD 1, PCINDEX 0, PCFIELD 1]'
CONTEXTS = {
 'known-append': [f'$ppaccess(P, {ARG1}) = (PPW)', f'$ppaccess(P, {ARG1} ++ [PCFIELD 0]) = (PPW)', f'$compiled_argument(P, {ARG1}) = eps', '$ppfunction_early(P.FUNCTIONS, ([102])) = (pfunction)', '$ppfunction_parameter_ref((pfunction), 1)', '~$ppfunction_parameter_ref((pfunction), 2)'],
 'forward-append': [f'$ppaccess(P, {ARG0}) = (PPF)', f'pcpath_arg = {ARG0}', f'$compiled_argument(P, {ARG0}) = [(CODEARG pcpath_arg)]'],
 'known-alias': [f'$ppaccess(P, {ARG2}) = (PPW)'],
 'known-globals': [f'$ppaccess(P, {ARG1}) = (PPR)'],
 'known-this': [f'$ppaccess(P, {ARG1}) = (PPW)'],
 'call-temporary-dimension': [f'$ppaccess(P, {ARG2}) = (PPW)', f'$ppaccess(P, {ARG2} ++ [PCFIELD 0]) = (PPR)'],
 'header-write': ['P.HEADERASSIGNED'],
 'header-append': ['P.HEADERASSIGNED'],
}
...
def main():
    ...
    files = [*modules, runner, adapter_path, types.PHP, Path(__file__),
             ROOT / 'tests/semantics/function_reference_cases.json',
             ROOT / 'frontend/worker.php', ROOT / '.tools/php-file.so']
    before = {str(p.relative_to(ROOT)): digest(p) for p in files}
    out = Path(tempfile.mkdtemp(prefix='function-reference-compiler-', dir=ROOT / '.tools'))
    ...
    (ROOT / 'coverage/semantics/function-reference-compiler.json').write_text(json.dumps(report, indent=2) + '\n')
