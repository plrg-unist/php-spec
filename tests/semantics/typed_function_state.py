...
SELECTION = ['byref-int-coercion-global-alias', 'byref-float-coercion-array-slot', 'typed-default-ref-fresh-cache', 'ref-warning-before-missing']
...
def main():
    ...
    out = Path(tempfile.mkdtemp(prefix='typed-function-state-', dir=ROOT / '.tools'))
    ...
    try:
        for name in SELECTION:
            directory = out / name
            directory.mkdir()
            source = typed_functions.CASES[name]
            ...
            if name == 'typed-default-ref-fresh-cache':
                checks += ['S_out.DEFAULTCACHE = [pdefaultcache]', 'pdefaultcache.VALUE = PSTRING ([50])', 'pdefaultcache.CLASS = PVSTRING true']
            else:
                checks += ['S_out.DEFAULTCACHE = eps']
            if name == 'byref-int-coercion-global-alias':
                checks += ['$lookup(S_out.ENV, [97]) = (n_a)', '$lookup(S_out.ENV, [98]) = (n_a)', 'S_out.STORE[n_a] = DEFINED (PINT 1)', 'n_a <- S_out.REFCELLS']
            elif name == 'typed-default-ref-fresh-cache':
                checks += ['$lookup(S_out.ENV, [115,97,118,101,100]) = (n_a)', 'S_out.STORE[n_a] = DEFINED (PARRAY n_array)', '$entry_lookup(S_out.ARRAYS[n_array].ITEMS, KINT 0) = (ALIAS n_first)', '$entry_lookup(S_out.ARRAYS[n_array].ITEMS, KINT 1) = (ALIAS n_second)', 'n_first =/= n_second', 'S_out.STORE[n_first] = DEFINED (PINT 9)', 'S_out.STORE[n_second] = DEFINED (PINT 7)']
            elif name == 'ref-warning-before-missing':
                checks += ['$lookup(S_out.ENV, [120]) = (n_a)', 'S_out.STORE[n_a] = DEFINED (PINT 1)', 'n_a <- S_out.REFCELLS']
            else:
                checks += ['$lookup(S_out.ENV, [97]) = (n_a)', 'S_out.STORE[n_a] = DEFINED (PARRAY n_array)', '$entry_lookup(S_out.ARRAYS[n_array].ITEMS, KINT 0) = (ALIAS n_cell)', 'S_out.STORE[n_cell] = DEFINED (PFLOAT $float_of_int(2))']
    ...
    (ROOT / 'coverage/semantics/typed-function-state.json').write_text(json.dumps(report, indent=2))
    return True
