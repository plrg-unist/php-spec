#!/usr/bin/env python3
"""Checked nullsafe property modes, facts, and static priority."""
...
CASES = json.loads((ROOT / 'tests/semantics/nullsafe_compiler_cases.json').read_text())


def run():
    before = types.syntax_validation.implementation_fingerprint()
    out = Path(tempfile.mkdtemp(prefix='nullsafe-compiler-', dir=ROOT / '.tools'))
    modules = [ROOT / p for p in json.loads((ROOT / 'spec/semantics/modules.json').read_text())]
    watched = [*modules, Path(__file__), ROOT / 'tests/semantics/nullsafe_compiler_cases.json']
    ...
    try:
        for row in CASES:
            ...
            premises = [f'P = $ppstart(139, {checked["fixture"]}, {filename})']
            if 'static_error' in row:
                premises.append('P.COMPLETION = PPCABRUPT (STATICERROR '
                                + json.dumps(row['static_error']) + ' '
                                + str(row['static_line']) + ')')
            elif 'line_checks' in row:
                premises.append('P.COMPLETION = PPCNORMAL')
                for check in row['line_checks']:
                    premises.append('$code_expression($compiled_operands(P, P.EXPRESSIONS), '
                                    + check['path'] + ') = '
                                    + f'(({check["line"]}, false))')
            else:
                premises.extend([
                    'P.COMPLETION = PPCNORMAL',
                    f'pcpath_nullsafe = {row["path"]}',
                    f'(pcpath_nullsafe, {row["mode"]}) <- P.ACCESS',
                    '$origin_node([P.FOLD.SOURCE], PORIGIN P.FOLD.SOURCE.ID pcpath_nullsafe) = '
                    '(NExprNullsafePropertyFetch expression_base phpType20_name metadata)',
                    '$code_expression($compiled_operands(P, P.EXPRESSIONS), pcpath_nullsafe) = '
                    f'(({row["code_line"]}, false))',
                ])
                if row['computed_name']:
                    premises.append('$code_expression($compiled_operands(P, P.EXPRESSIONS), '
                                    'pcpath_nullsafe ++ [PCFIELD 1]) = '
                                    f'(({row["code_line"]}, false))')
