...
SELECTION = ['variadic-returned-reference-tail', 'variadic-tail-singleton-after-cleanup']

PREFIX = d.PREFIX + f.PREFIX + r'''
dec $variadic_stage_head(ptask*) : bool
def $variadic_stage_head((VARIADIC_RECEIVE porigin 1) :: ptask*) = true
def $variadic_stage_head(ptask*) = false -- otherwise
dec $variadic_stage_cut(pstate, nat, nat) : nat
def $variadic_stage_cut(S, n, n_left) = n -- if $variadic_stage_head(S.TODO)
def $variadic_stage_cut(S, n, n_left) = $variadic_stage_cut($drive_steps(S[.COMPLETION = NORMAL], 1), $(n + 1), n_rest)
  -- if ~$variadic_stage_head(S.TODO)
  -- if $(n_left > 0)
  -- if n_rest = $(n_left - 1)
...
def main():
    ...
    out = Path(tempfile.mkdtemp(prefix='variadic-state-', dir=ROOT / '.tools'))
    ...
    try:
        for name in SELECTION:
            ...
            checks += ['n_return = $variadic_stage_cut(S, 0, 256)',
                       'S_return = $drive(S, n_return)', '$variadic_stage_head(S_return.TODO)',
                       'S_return.CURRENT = (pcallcontext)', 'pcallcontext.ARGC = 2',
                       'pcallcontext.EXTRA = [REFERENCE n_first, REFERENCE n_second]',
                       '$lookup(S_return.ENV, [120,115]) = (n_tail_cell)',
                       'S_return.STORE[n_tail_cell] = DEFINED (PARRAY n_tail)',
                       'S_return.ARRAYS[n_tail].ITEMS = [ENTRY (KINT 0) (ALIAS n_first)]',
                       '$($heap_owners($heap_graph(S_return), HCELL n_first) >= 2)',
            ...
            if name == 'variadic-returned-reference-tail':
                checks += ['$lookup(S_out.ENV, [97]) = (n_a)', '$lookup(S_out.ENV, [98]) = (n_b)',
                           'S_out.STORE[n_b] = DEFINED (PINT 4)',
                           '$entry_lookup(S_out.ARRAYS[n_r_array].ITEMS, KINT 0) = (ALIAS n_a)',
                           '$entry_lookup(S_out.ARRAYS[n_v_array].ITEMS, KINT 0) = (ALIAS n_a)',
                           '$entry_lookup(S_out.ARRAYS[n_r_array].ITEMS, KINT 1) = (ALIAS n_b)',
                           '$entry_lookup(S_out.ARRAYS[n_v_array].ITEMS, KINT 1) = (ALIAS n_b)']
            else:
                checks += ['S_return.GLOBALTABLE = (psymboltable)', '$lookup(psymboltable.ENV, [97]) = eps',
            ...
            budgets = [str(n) for n in [*range(33), 64, 128]] + ['$(n_return - 1)', 'n_return'] + [f'$(n_return + {n})' for n in range(1, 9)]
            ...
            full_indices = [0, 1, 7, 16, 32, 33, 34, 35, 36, 37, 41, 44]
    ...
    (ROOT / 'coverage/semantics/variadic-state.json').write_text(json.dumps(report, indent=2))
    return True
