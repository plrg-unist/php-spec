...
import foreach as f
import function_references as function_calls
...
SELECTION = [
    'list-edge-call-nested', 'list-edge-detached-list-return',
    'acquire-edge-call-reference-assignment', 'acquire-edge-call-list-target',
    'list-edge-ref-rhs-composition', 'reference-before-later-error',
]
...
def main():
    ...
    out = Path(tempfile.mkdtemp(prefix='function-reference-state-', dir=ROOT / '.tools'))
    ...
    try:
        for name in SELECTION:
            directory = out / name
            directory.mkdir()
            source = function_calls.CASES[name]
            ...
            checks = ['S_initial = ' + initial, 'S_initial.COMPLETION = BUDGET', 'S = S_initial[.COMPLETION = NORMAL]',
                      ...
                      '$heap_valid($heap_graph(S_out))', '$call_valid(S_out)']
            if name == 'list-edge-ref-rhs-composition':
                checks += ['$lookup(S_out.ENV, [97]) = $lookup(S_out.ENV, [98])',
                           '$lookup(S_out.ENV, [97]) = (n_alias)',
                           'S_out.STORE[n_alias] = DEFINED (PARRAY n_array)',
                           '$entry_lookup(S_out.ARRAYS[n_array].ITEMS, KINT 0) = (DIRECT (PINT 9))']
            if name == 'acquire-edge-call-reference-assignment':
                checks += ['$lookup(S_out.ENV, [97]) = (n_alias)',
                           'n_alias <- S_out.REFCELLS', '(HCELL n_alias) <- S_out.ALLOCATIONS',
                           'S_out.STORE[n_alias] = DEFINED (PINT 9)',
                           '|S_out.ALLOCATIONS| = $(|S.ALLOCATIONS| + 1)']
            cuts = [*range(33), 64, 128]
            for budget in sorted(set(cuts) | {n + 1 for n in cuts}):
                checks += [f'S_b{budget} = $drive(S, {budget})']
            for budget in cuts:
                checks += [f'$resume_destructuring(S_b{budget}, 1) = S_b{budget + 1}',
                           f'$heap_valid($heap_graph(S_b{budget}))', f'$call_valid(S_b{budget})',
                           f'S_b{budget}.REQUEST = S.REQUEST', f'S_b{budget}.HTTPROOTS = S.HTTPROOTS']
    ...
    (ROOT / 'coverage/semantics/function-reference-state.json').write_text(json.dumps(report, indent=2))
    return True
