...
SELECTION = ['return-ref-temporary-call-leaf', 'return-reference-assignment-value-class']

PREFIX = d.PREFIX + f.PREFIX + r'''
dec $reference_stage_head(ptask*) : bool
def $reference_stage_head((RETURN_REF_FETCH z) :: ptask*) = true
def $reference_stage_head((RETURN_REF_VALUE z) :: ptask*) = true
def $reference_stage_head(ptask*) = false -- otherwise
dec $reference_stage_cut(pstate, nat, nat) : nat
def $reference_stage_cut(S, n, n_left) = n -- if $reference_stage_head(S.TODO)
def $reference_stage_cut(S, n, n_left) = $reference_stage_cut($drive_steps(S[.COMPLETION = NORMAL], 1), $(n + 1), n_rest)
  -- if ~$reference_stage_head(S.TODO)
  -- if $(n_left > 0)
  -- if n_rest = $(n_left - 1)
...
def main():
    ...
    out = Path(tempfile.mkdtemp(prefix='reference-return-state-', dir=ROOT / '.tools'))
    ...
    try:
        for name in SELECTION:
            directory = out / name
            directory.mkdir()
            source = reference_returns.CASES[name]
            ...
            if name == 'return-ref-temporary-call-leaf':
                checks += ['$lookup(S_out.ENV, [120]) = (n_x)', 'S_out.STORE[n_x] = DEFINED (PINT 3)',
                           'n_x <- S_out.REFCELLS', '$heap_owners($heap_graph(S_out), HCELL n_x) = 1']
            else:
                checks += ['$lookup(S_out.ENV, [120]) = (n_x)', '$lookup(S_out.ENV, [121]) = (n_y)',
                           '$lookup(S_out.ENV, [114]) = (n_y)', 'n_x =/= n_y',
                           'S_out.STORE[n_x] = DEFINED (PINT 1)', 'S_out.STORE[n_y] = DEFINED (PINT 3)',
                           'n_y <- S_out.REFCELLS']
            checks += ['n_return = $reference_stage_cut(S, 0, 128)',
                       'S_return = $drive(S, n_return)', '$reference_stage_head(S_return.TODO)']
    ...
    (ROOT / 'coverage/semantics/reference-return-state.json').write_text(json.dumps(report, indent=2))
    return True
