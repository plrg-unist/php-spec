...
SELECTION = ['strict-reference-default-cow','strict-nested-call-owner']
...
def main():
    ...
    out = Path(tempfile.mkdtemp(prefix='strict-declaration-state-', dir=ROOT / '.tools'))
    ...
    try:
        for name in SELECTION:
            directory = out / name
            directory.mkdir()
            source = strict_declarations.CASES[name]
            ...
            checks += ['S_out.DEFAULTCACHE = eps', '$code_at(S_out.CODE, 0) = (pcode)', 'pcode.STRICT']
    ...
    (ROOT / 'coverage/semantics/strict-declaration-state.json').write_text(json.dumps(report, indent=2))
    return True
