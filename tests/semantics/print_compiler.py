#!/usr/bin/env python3
"""Print compiler constants, retained effects, static restrictions and source lines."""
import base64
import hashlib
import json
...
CASES = {
    ...
    'multiline': '<?php\nprint (\n  1 +\n  2\n);\n',
    'refsend': '<?php\nfunction f(&$x) {}\nf(print "x");\n',
    'refreturn': '<?php\nfunction &f() { return print "x"; }\n',
    'badwrite': '<?php\n(print "x")[0] = 2;\n',
    'badread': '<?php\nprint $a[];\n',
    'badconstant': '<?php\nconst X = print "x";\n',
}
CONTEXTS = {}
for name in ('literal', 'nested', 'multiline'):
    CONTEXTS[name] = [
        'pcpath_print = [PCINDEX 0, PCFIELD 0]',
        '$occurrence_node(P.FOLD.SOURCE.OCCURRENCES, pcpath_print) = (NExprPrint expression_child metadata_print)',
        '$ppaccess(P, pcpath_print) = (PPR)',
        '$ppaccess(P, pcpath_print ++ [PCFIELD 0]) = (PPR)',
        '(PPCEFFECT pcpath_print) <- P.EXPRESSIONS',
        '~$ppsend_var(NExprPrint expression_child metadata_print)',
        '~$ppreturn_variable(NExprPrint expression_child metadata_print)',
        '~$ppreturn_call(NExprPrint expression_child metadata_print)',
        '$pptemporary(NExprPrint expression_child metadata_print)',
        '$ppconstants(P) = PPCCONSTANTS (pcpath_constant, pvalue_constant)*',
        '$ppconstant_at((pcpath_constant, pvalue_constant)*, pcpath_print) = (PINT 1)',
        '$pffact(P.FOLD.FACTS, pcpath_print) = eps',
    ]
CONTEXTS['literal'] += ['$code_expression($compiled_operands(P, P.EXPRESSIONS), pcpath_print) = ((2, true))']
CONTEXTS['multiline'] += ['$code_expression($compiled_operands(P, P.EXPRESSIONS), pcpath_print) = ((4, true))']
CONTEXTS['nested'] += [
    '(PPCEFFECT (pcpath_print ++ [PCFIELD 0])) <- P.EXPRESSIONS',
    '$effect_select($compiled_operands(P, P.EXPRESSIONS), $compiled_operands(P, P.EXPRESSIONS), eps) = [CODEEFFECT pcpath_print]',
]
CONTEXTS['folded'] = [
...
CONTEXTS['refsend'] = [
    'pcpath_print = [PCINDEX 1, PCFIELD 0, PCFIELD 1, PCINDEX 0, PCFIELD 1]',
    '$occurrence_node(P.FOLD.SOURCE.OCCURRENCES, pcpath_print) = (NExprPrint expression_child metadata_print)',
    '~$ppsend_var(NExprPrint expression_child metadata_print)',
    '$ppaccess(P, pcpath_print) = (PPR)',
    '(PPCEFFECT pcpath_print) <- P.EXPRESSIONS',
...
def main():
    ...
    out = Path(tempfile.mkdtemp(prefix='print-compiler-', dir=ROOT / '.tools'))
    ...
    report = {'scope': 'Print compilation, native lint diagnostics, constant-result effects, reference category and source lines; runtime outcomes are checked separately.',
    ...
    (ROOT / 'coverage/semantics/print-compiler.json').write_text(json.dumps(report, indent=2) + '\n')
